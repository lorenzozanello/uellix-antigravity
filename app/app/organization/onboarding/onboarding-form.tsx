...
/**
 * Formulario de configuración inicial de la organización.
 *
 * F0-03: este componente sólo se renderiza para roles que pueden enviarlo
 * (`super_admin` y `organization_admin`). La comprobación vive en el Server
 * Component `page.tsx`, de modo que un miembro sin permiso nunca ve un
 * formulario que su envío rechazaría.
 */
export function OnboardingForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // F0-02: antes era router.push('/app'), una ruta inexistente que
      // devolvía un 404 en inglés tras guardar correctamente los datos.
    ...
    <div className="min-h-screen bg-[var(--uellix-paper)] flex items-center justify-center p-4">
      <div className="w-full max-w-lg bg-white rounded-2xl shadow-sm border border-[#0F172A]/10 p-8 md:p-12">
        ...
        {error && (
          <div
            role="alert"
            className="mb-6 p-4 bg-red-50 text-red-700 text-sm rounded-lg border border-red-200"
          >
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label htmlFor="country" className="block text-sm font-semibold text-[#0F172A] mb-2 flex items-center gap-2">
              <Globe className="w-4 h-4 text-slate-400" aria-hidden="true" />
              País de Operación Principal
            </label>
            <select id="country" name="country" required className="w-full h-11 px-4 rounded-lg border border-slate-200 bg-white text-sm focus:border-uellix-orange focus:ring-1 focus:ring-uellix-orange outline-none transition-colors">
          ...
          <div>
            <label htmlFor="sector" className="block text-sm font-semibold text-[#0F172A] mb-2 flex items-center gap-2">
              <Building className="w-4 h-4 text-slate-400" aria-hidden="true" />
              Sector Principal
            </label>
            <select id="sector" name="sector" required className="w-full h-11 px-4 rounded-lg border border-slate-200 bg-white text-sm focus:border-uellix-orange focus:ring-1 focus:ring-uellix-orange outline-none transition-colors">
          ...
          <div>
            <label htmlFor="baseCurrency" className="block text-sm font-semibold text-[#0F172A] mb-2 flex items-center gap-2">
              <DollarSign className="w-4 h-4 text-slate-400" aria-hidden="true" />
              Moneda Base (para cálculos SROI)
            </label>
            <select id="baseCurrency" name="baseCurrency" required className="w-full h-11 px-4 rounded-lg border border-slate-200 bg-white text-sm focus:border-uellix-orange focus:ring-1 focus:ring-uellix-orange outline-none transition-colors">
