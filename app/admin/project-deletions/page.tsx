...
export default function ProjectDeletionsPage() {
  ...
  useEffect(() => {
    const loadDeletionRequests = async () => {
      try {
        ...
        setRequests(records as unknown as DeletionRequest[]);
      } catch (error) {
        toast({
          title: 'Error',
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center p-8">
        <p className="text-muted-foreground">Cargando solicitudes de eliminación...</p>
      </div>
    );
  }
