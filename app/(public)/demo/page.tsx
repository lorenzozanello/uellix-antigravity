  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto max-w-5xl grid grid-cols-1 gap-14 lg:grid-cols-[0.85fr_1fr] lg:items-start">
        {/* Left — narrative */}
        <div>
          <span className="inline-flex items-center gap-2 rounded-full border border-[#fc4c0d]/25 bg-[#fc4c0d]/8 px-3.5 py-1.5 text-[11px] font-semibold text-[#B85200] mb-7 font-ibm-plex-mono tracking-[0.14em] uppercase">
            <span className="h-1.5 w-1.5 rounded-full bg-[#fc4c0d] shrink-0" aria-hidden="true" />
            Demo privada
          </span>
          <h1 className="font-sora font-semibold tracking-[-0.02em] text-[#0F172A] mb-6 text-[clamp(2.2rem,4.5vw,3.2rem)] leading-[1.05]">
            ...
            <span className="text-[#fc4c0d]">análisis defendible.</span>
          </h1>
          ...
          <ul aria-label="Incluido con Uellix" className="flex flex-col gap-2.5 mb-8">
            {benefits.map((benefit) => (
              <li key={benefit} className="flex items-center gap-2.5 text-sm text-[#0F172A] font-manrope">
                <CheckCircle2 className="h-4 w-4 shrink-0 text-[#fc4c0d]" aria-hidden="true" />
                {benefit}
              </li>
            ))}
          </ul>
          <p className="text-xs text-[#64748B] leading-relaxed font-manrope border-l-2 border-[#fc4c0d]/30 pl-3 max-w-sm">
