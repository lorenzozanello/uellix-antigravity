...
BEGIN
  IF NOT (SELECT rolsuper FROM pg_roles WHERE rolname = current_user) THEN
    RAISE EXCEPTION 'stella_0015 aborted: must run as a SUPERUSER (current_user=%). It drops and republishes functions owned by uellix_cap_stella_ticket, which uellix_owner does not own.', current_user;
  END IF;
