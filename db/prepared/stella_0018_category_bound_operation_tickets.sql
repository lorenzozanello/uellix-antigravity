...
BEGIN
  IF NOT (SELECT rolsuper FROM pg_roles WHERE rolname = current_user) THEN
    RAISE EXCEPTION 'stella_0018 aborted: must run as a SUPERUSER (current_user=%). It withdraws grants issued by earlier packages and republishes SECURITY DEFINER bodies owned by a capability role.', current_user;
  END IF;
