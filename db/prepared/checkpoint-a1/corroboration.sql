...
-- compares bytes. The 37 witness arms below come from
...
SELECT jsonb_pretty(jsonb_build_object(
  ...
  -- GAP C. 11 packages, 37 witnesses, each measured on its own.
  'packageObservations', jsonb_build_array(
    ...
    jsonb_build_object(
      'packageId', 'stella_0019_storage_write_roles',
      'witnesses', jsonb_build_object(
      'routine-body:public.can_write_evidence_object(text, uuid):impact_manager', EXISTS (SELECT 1 FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('public.can_write_evidence_object(text, uuid)') AND position('impact_manager' in pg_catalog.regexp_replace(pg_catalog.pg_get_functiondef(p.oid), '--[^\n]*', '', 'g')) > 0)
      )))
));
