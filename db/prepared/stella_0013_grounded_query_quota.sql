...
BEGIN
  IF NOT (SELECT rolsuper FROM pg_roles WHERE rolname = current_user) THEN
    RAISE EXCEPTION 'stella_0013 aborted: must run as a SUPERUSER (current_user=%). It creates a role, transfers function ownership to it, and grants USAGE on schema auth — none of which uellix_owner can do.', current_user;
  END IF;
