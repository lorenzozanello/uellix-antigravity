...
BEGIN
  ...
  IF NOT (SELECT rolsuper FROM pg_roles WHERE rolname = current_user) THEN
    RAISE EXCEPTION 'stella_0019 aborted: must run as a SUPERUSER (current_user=%). It republishes a function owned by uellix_owner, which requires becoming that role and holding CREATE on schema public for the length of one statement.', current_user;
  END IF;
...
-- ============================================================
-- 1. can_write_evidence_object, republished in place (owner window)
-- ============================================================
-- IDENTICAL to unit 41 in every respect a caller can observe — schema, name,
-- argument names and types, return type, language, SECURITY DEFINER, empty
-- search_path, the not-yet-migrated guard, the path parse, the EXCEPTION
-- handler — except for the role list, which becomes the four
-- `evidence_items_insert` already names.
--
-- THE OWNER WINDOW IS THE POINT, AND IT IS THE SAME ONE STAGING USES.
-- A superuser could issue the CREATE OR REPLACE directly and PostgreSQL would
-- keep the owner anyway. Doing it through `SET ROLE uellix_owner` instead means
-- the statement that runs here is the statement that runs on managed Supabase,
-- where the installer is `uellix_migrator` — not a superuser, not the owner,
-- and able to replace this function only by becoming uellix_owner first.
-- Measured, PG 17.6: uellix_migrator without the window fails at `permission
-- denied for schema public`, because PostgreSQL checks CREATE on the containing
-- namespace against the role EXECUTING the statement.
