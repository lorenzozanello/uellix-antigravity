...
BEGIN
  IF NOT (SELECT rolsuper FROM pg_roles WHERE rolname = current_user) THEN
    RAISE EXCEPTION 'grounding_0003 aborted: must run as a SUPERUSER (current_user=%). It transfers function ownership to uellix_cap_grounding, which requires it.', current_user;
  END IF;
