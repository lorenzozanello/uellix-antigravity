...
BEGIN
  IF NOT (SELECT rolsuper FROM pg_roles WHERE rolname = current_user) THEN
    RAISE EXCEPTION 'stella_0014 aborted: must run as a SUPERUSER (current_user=%). It creates a role, transfers ownership to it and grants USAGE on schema auth — none of which uellix_owner can do.', current_user;
  END IF;
...
ALTER ROLE uellix_cap_stella_ticket
...
GRANT USAGE ON SCHEMA auth TO uellix_cap_stella_ticket;
GRANT EXECUTE ON FUNCTION auth.uid() TO uellix_cap_stella_ticket;
