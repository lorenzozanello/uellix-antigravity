...
BEGIN
  IF NOT (SELECT rolsuper FROM pg_roles WHERE rolname = current_user) THEN
    RAISE EXCEPTION 'stella_0017 aborted: must run as a SUPERUSER (current_user=%). It revokes privileges granted by the baseline restore and transfers function ownership to capability roles — neither of which uellix_owner can do.', current_user;
  END IF;
