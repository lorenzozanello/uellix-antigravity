...
export const BASELINE_UNITS: readonly BaselineUnit[] = [
  ...
  { ordinal: 82, id: '0069_fib_fibdb052_p1_indexes.sql', kind: D, file: 'db/migrations/0069_fib_fibdb052_p1_indexes.sql', sha256: 'c7808ef43221662a27d7a227a6271f34cea09f1167cf70597948924fefcab379', dependsOn: ['0068_commercial_account_ce1.sql'], ...PLAIN_DDL, reapply: 'idempotent',
    rollback: 'Every index is CREATE INDEX IF NOT EXISTS; re-running converges. Dropping is safe and lossless.',
    expect: {} },
  // CL-1 (CUSTOMER_LIFECYCLE_CL1_EXECUTION_AUTHORITY_v1.0.0.json, HPO-ODS-W2-28):
  // the L0 account-class legal-acceptance substrate. CREATE TABLE for T1
  // (legal_instruments), T2 (legal_instrument_versions) and T3
  // (account_legal_acceptances) -- generated by drizzle-kit generate -- plus
  // hand-authored RLS (ENABLE on T1/T2, ENABLE+FORCE on T3), four
  // uellix_forbid_mutation() append-only triggers, one new function
  // (enforce_account_legal_acceptance_invariants, I-T3-4/I-T3-5) and one
  // additive audit_logs INSERT policy admitting exactly the org-less
  // acceptance verb (ORGLESS_AUDIT_AUTHORITY, F-AO-15). Depends on 0030 for
  // the reused trigger function and on 0031/0042/0067 for auth.uid() and the
  // two sibling audit_logs INSERT policies this one sits beside, unedited.
  {
    ordinal: 83,
    id: '0070_customer_lifecycle_cl1_legal_acceptance.sql',
    kind: D,
    file: 'db/migrations/0070_customer_lifecycle_cl1_legal_acceptance.sql',
    sha256: 'd12995439cabf5e1068f1ff59212cde7005524fcdce76530c1ff240e59d77728',
    dependsOn: [
      '0069_fib_fibdb052_p1_indexes.sql',
      '0030_immutability.sql',
      '0031_rls_core.sql',
      '0042_fib_audit_insert_policy.sql',
    ...
    reapply: 'destructive-on-reapply',
    managedNote:
      'CL-1: CREATE TABLE for legal_instruments, legal_instrument_versions and account_legal_acceptances ' +
      '(no IF NOT EXISTS, forward-only). RLS ENABLED on all three (T3 also FORCED); five policies -- ' +
      'read-open/write-closed SELECT on T1/T2, self-scoped SELECT+INSERT on T3, and one additive ' +
      'audit_logs INSERT policy for the org-less acceptance verb, alongside 0042 and 0067 unedited. ' +
      'Four append-only triggers reuse uellix_forbid_mutation() (0030); one new trigger function ' +
      'enforces the cross-table T3 invariants no CHECK constraint can express.',
    rollback:
      'Forward-only: CREATE TABLE/ADD CONSTRAINT/CREATE INDEX have no IF NOT EXISTS guard and no ' +
      'reverse script, recovered by DESTROY_AND_REPROVISION. The five CREATE POLICY and four ' +
      'CREATE TRIGGER statements are individually idempotent (guarded DROP IF EXISTS first).',
    expect: {
      referencesAuthSchema: true,
      rlsEnabledTableCount: 3,
      policiesCreatedCount: 5,
      functionsCreatedCount: 1,
      triggersCreatedCount: 4,
