...
/** Every [start, end) region of `"repo": <quoted value>` in the serialized text, for an adjudicated value. */
function quotedRegions(text: string, values: Set<string>): [number, number][] {
  const regions: [number, number][] = []
  for (const v of values) {
    const quoted = JSON.stringify(v)
    for (const key of ['"repo": ', '"repo":']) {
...
export function scanEvidenceText(text: string, file: string, isAdjudicatedValue: (v: string) => boolean): Omit<Ec1Report, 'files'> {
  ...
  // A. serialized
  const regions = quotedRegions(text, allowed)
...
export function scanEvidenceFiles(files: readonly string[], isAdjudicatedValue: (v: string) => boolean): Ec1Report {
  if (files.length === 0) throw new Error('STOP_EVIDENCE_SCAN_EMPTY_SET: no files to scan')
  let leaves = 0
  let explained = 0
  const unexplained: Ec1Finding[] = []
  for (const file of files) {
    const r = scanEvidenceText(readFileSync(file, 'utf8'), path.basename(file), isAdjudicatedValue)
...
function main(argv: string[]): number {
  ...
  try {
    ...
    // executor's markers can be honoured (see the header).
    report = scanEvidenceFiles(argv, () => true)
  } catch (e) {
