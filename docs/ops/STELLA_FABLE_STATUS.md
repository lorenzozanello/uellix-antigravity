...
| WS2 Advisor UX | **INTEGRADO** (merge `0d0791a`) | `moonshot/ws2-advisor-ux` | Panel contextual con fuentes legibles (RK-03 cerrado), ciclo completo con undo LIFO global + confirmación de staleness, composer sin DOM imperativo, taxonomía 12 códigos (RK-13), DISABLED como prop (inerte), a11y (RK-34). Invariante sin-escritura-automática verificado por auditor. Pendiente Ola 3: adapter onDecision↔recordStellaDecision (contratos difieren — reconciliar deliberadamente) |
...
| WS3b Persistencia & DB security | **INTEGRADO** (merge `3e967d0`) | `moonshot/ws3b-persistence` | stella_0002/0003 preparados con rollback (RK-04→G2), audit_logs completo (RK-12), Sentry sanitizado (RK-23), decisiones dormante org-verificada (RK-11 parcial, flag+G2), casos RLS listos (G3), retención DP-04 draft |
| WS6 Roles & Eval | **INTEGRADO** (merge `8f39d2a` + wiring `791fa9b`) | `moonshot/ws6-roles-eval` | Contextos por rol con linkage de outcomes (RK-17 cerrado), contratos versionados con pins de keys+enums, `pnpm eval:roles` 5/5, fixture conectada (RK-27), G4. Pendiente coordinador: pasar `role` en reviewer.ts:87 (tras merge WS3b). Reformulation fuera por decisión de producto (DP en DECISIONS) |
