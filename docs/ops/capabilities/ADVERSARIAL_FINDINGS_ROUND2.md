...
| A2-F13 / B2-F13 | El rollback de CAP-03 seguía revocando una membresía que el forward ya no concede | **NO cerrado.** `stella_0008_rollback.sql` sigue conteniendo `EXECUTE 'REVOKE uellix_cap_stripe FROM uellix_owner'`. La línea es inocua (un `REVOKE` de una membresía inexistente no falla) pero la afirmación de que se había eliminado era falsa, y la detectó la revisión adversarial de la ronda 3. Reabierto como higiene, no como riesgo |
