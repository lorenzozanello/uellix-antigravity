...
describe('the baseline manifest describes the corpus that is actually checked in', () => {
  ...
  // (CREATE TABLE x3, generated by drizzle-kit generate) = 83 units, 71
  // Drizzle. It is a DRIZZLE unit — kind records which checked-in SET a unit
  // comes from (db/migrations/), not whether it also carries hand-authored
  // RLS/triggers/policy — so the `policy` and `supabase` pins are unmoved.
  // CL-1 presentation-binding repair (independent-audit continuation): +
  // 0071_customer_lifecycle_cl1_content_bytes.sql (ADD COLUMN, generated by
  // drizzle-kit generate) = 84 units, 72 Drizzle.
  // L1 ORGANIZATION_COMMERCIAL_ACCEPTANCE (HPO-ODS-W2-29): +
  // 0072_customer_lifecycle_l1_organization_commercial_acceptance.sql (CREATE
  // TABLE organization_commercial_acceptances, generated by drizzle-kit
  // generate, plus hand-authored RLS/triggers) = 85 units, 73 Drizzle. Same
  // reasoning as every predecessor wave: `kind` records which checked-in SET a
  // unit comes from (db/migrations/), not whether it also carries
  // hand-authored SQL — so the `policy` and `supabase` pins are unmoved again.
  // COMMERCIAL ACCOUNT CE-3 (HPO-ODS-W2-30): +
  // 0073_commercial_account_ce3_entitlement_grants.sql (CREATE TABLE
  // entitlement_grants, generated by drizzle-kit generate, plus hand-authored
  // RLS, a SECURITY DEFINER evaluator and an append-only trigger) = 86 units,
  // 74 Drizzle. Same reasoning again, and for the same reason: the unit is
  // checked in under db/migrations/, so `policy` and `supabase` stay put.
  it('has 86 units: 74 Drizzle, 2 Supabase, 10 policies', () => {
    expect(BASELINE_UNITS).toHaveLength(86)
    const byKind = (k: string) => BASELINE_UNITS.filter((u) => u.kind === k).length
    expect(byKind('drizzle-migration')).toBe(74)
    expect(byKind('supabase-migration')).toBe(2)
    expect(byKind('policy')).toBe(10)
  })

  it('numbers ordinals 1..86 contiguously, and BASELINE_ORDER is derived from them', () => {
    ...
    expect(new Set(BASELINE_ORDER).size).toBe(86)
  })
