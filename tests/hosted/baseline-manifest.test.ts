...
describe('the baseline manifest describes the corpus that is actually checked in', () => {
  ...
  it('has 83 units: 71 Drizzle, 2 Supabase, 10 policies', () => {
    expect(BASELINE_UNITS).toHaveLength(83)
    const byKind = (k: string) => BASELINE_UNITS.filter((u) => u.kind === k).length
    expect(byKind('drizzle-migration')).toBe(71)
    expect(byKind('supabase-migration')).toBe(2)
    expect(byKind('policy')).toBe(10)
  })

  it('numbers ordinals 1..83 contiguously, and BASELINE_ORDER is derived from them', () => {
    ...
    expect(new Set(BASELINE_ORDER).size).toBe(83)
  })
