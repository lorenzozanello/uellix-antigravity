...
beforeEach(async () => {
  ...
  vi.mocked(requireOrganizationAccess).mockResolvedValue({ organization: ORG, user: { id: 'user-1' }, membership: { role: 'impact_manager' } } as any)
})
...
async function expectForkWithReset(): Promise<any> {
...
describe('ten FIBC-013 material categories — each forks an approved version (R-B2-09)', () => {
  ...
  it.each(EDIT_PATH_CASES)('$category — changing $key forks; V1 untouched; V2 inherits no approval, no rubric, no determination', async ({ key, value }) => {
    ...
    if (key === 'consultationDate') expect(new Date(v2.consultationDate).toISOString().slice(0, 10)).toBe(value)
