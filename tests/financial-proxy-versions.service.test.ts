...
/** desc(column) carries a literal ' desc' chunk; this is the slice of the order expression read below. */
interface DrizzleOrderExpr {
  readonly queryChunks?: readonly { readonly value?: readonly unknown[] }[]
}
    ...
    select: vi.fn().mockImplementation(() => ({
      from: vi.fn().mockImplementation(() => {
        const query: MockQuery = {
          ...
          orderBy: vi.fn().mockImplementation((orderExpr: unknown) => {
            ...
            const isDesc = (orderExpr as DrizzleOrderExpr | undefined)?.queryChunks?.some((c) => c?.value?.[0] === ' desc')
            const data = query.__filtered ?? mockDbData.financialProxyVersions
            query.__filtered = [...data].sort((a, b) => (isDesc ? (b.ordinal ?? 0) - (a.ordinal ?? 0) : (a.ordinal ?? 0) - (b.ordinal ?? 0)))
            return query
          }),
          ...
          then: (cb: (rows: MockRow[]) => unknown) => Promise.resolve(cb(query.__filtered ?? mockDbData.financialProxyVersions)),
        }
        return query
      }),
    })),
