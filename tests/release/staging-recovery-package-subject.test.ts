...
//
// tests/release/staging-recovery-package-subject.test.ts
// STAGING RECOVERY — PACKAGE SUBJECT bound to the protected canonical candidate,
// and the introducing view pinned to git (Recovery amendment v1.0.6, owner v1.0.4).
//
// Authority:
//   docs/ops/release/STAGING_RECOVERY_EXECUTION_AUTHORITY_AMENDMENT_v1.0.6.json
//   docs/ops/owner-ratifications/STAGING_RECOVERY_OWNER_DECISIONS_v1.0.4.json
//   docs/ops/release/STAGING_RECOVERY_EXECUTION_TEST_MANIFEST_AMENDMENT_v1.0.6.json
//
// WHAT THIS FILE IS. The reference interpreter of SECTION_G3 (the complete
// effective contract: v1.0.5 SECTION_F3 plus the SECTION_G3_DELTA changes): the
// V3 closure derivation with the self-referential-closure STOP, the controlled
// git read environment, store integrity, record validation (adjudications
// W1..W13, attempts A1..A12), ever-added history, canonical-store monotonicity,
// the provider-protection evaluator, attempt states, the package-subject
// reservation rule and the consumer rule. Subjects are derived from the git
// objects of their candidate commit; the introducing view is the git commit
// that adds the artifact. Every history-sensitive case runs on REAL git
// repositories built in a private temporary directory.
//
...
// measurements of v1.0.5/v1.0.6 or explicit fixtures) and mutates no branch
...
const PATHS = {
  v106: 'docs/ops/release/STAGING_RECOVERY_EXECUTION_AUTHORITY_AMENDMENT_v1.0.6.json',
  manifest106: 'docs/ops/release/STAGING_RECOVERY_EXECUTION_TEST_MANIFEST_AMENDMENT_v1.0.6.json',
  owner104: 'docs/ops/owner-ratifications/STAGING_RECOVERY_OWNER_DECISIONS_v1.0.4.json',
  self: 'tests/release/staging-recovery-package-subject.test.ts',
  battery: 'tests/release/staging-recovery-package-subject.mutation-battery.mjs',
...
const BASE_COMMIT = '76a3d4c2f96444604434146e1e1d1a81ccf07aff'
/** Every file of the Recovery chain that must stay byte-identical to its blob at the base. */
const IMMUTABLE = [
  PATHS.v100, PATHS.v101, PATHS.v102, PATHS.v103, PATHS.v104, PATHS.v105,
...
interface Pin { path: string; introducing_commit: string; introducing_tree: string }
interface V106 {
  SECTION_G2_OWNER_DECISION_BINDING: { owner_record: string; owner_record_blob: string }
  SECTION_G3_DELTA: { changed_paths: string[] }
  SECTION_G3_PACKAGE_SUBJECT_CONTRACT: Contract
  SECTION_G4_CURRENT_AND_NEXT_SUBJECT: { subject_at_lane_start: { candidate_sha: string; candidate_tree: string } }
  SECTION_G5_INTRODUCING_VIEW_PIN: { pins: Pin[] }
  SECTION_G6_CONSUMER_SEMANTICS: { cases: { case: string; outcome: string }[] }
  SECTION_G10_PRECEDENCE_ANCHORS: { anchors_changed: Record<string, Anchor[]> }
  SECTION_G11_SUPERSEDED_LOCI: { chain: string; topic: string; locus: string }[]
  SECTION_G14_WRITE_SET: string[]
  SECTION_G15_PRESERVATION: { AUTHORIZED_OPERATIONS_DELTA: { added: unknown[]; removed: unknown[]; modified: unknown[] } }
  final_state: string
}

/** v1.0.5: known subjects, registry, write-set rules and the F3 the delta is measured against. */
const PRIOR = readJson<V105>(PATHS.v105)
const AUTH = readJson<V106>(PATHS.v106)
const C = (a: V106 = AUTH): Contract => a.SECTION_G3_PACKAGE_SUBJECT_CONTRACT
const WRITE_SET = AUTH.SECTION_G14_WRITE_SET.map((w) => w.split(' ')[0])
const RETIRED_ROOT = PRIOR.SECTION_F7_MATERIALIZATION_AND_RESERVATION_WRITE_SET.retired_occurrence_root
const SELF_REF = 'self-referential closure'
/** SECTION_G3.subject_identity_derivation.path_encoding: TAB/LF in a path would make the S8 digest ambiguous. */
const UNENCODABLE = 'unencodable path'
const KS = (id: string): KnownSubject => {
  const s = PRIOR.SECTION_F5_KNOWN_SUBJECTS_V3.subjects.find((x) => x.id === id)
...
/** SECTION_G5: the unique commit of the history of `ref` that ADDS `rel` (controlled read). */
function introducingCommitOf(rel: string, cwd = ROOT, ref = 'HEAD'): string {
  if (isShallow(cwd)) throw new Error('shallow repository: the introducing commit is unverifiable')
  const found = text(cgit(cwd, ['log', '--full-history', '--diff-filter=A', '--format=%H', ref, '--', `:(top,literal)${rel}`])).split('\n').filter(Boolean)
  if (found.length !== 1) throw new Error(`introducing commit of ${rel}: ${found.length} candidates (is it committed?)`)
  return found[0]
}
/** Owner v1.0.4: the introducing view is the tree of that git commit, never worktree bytes or the current head. */
function introducingViewOf(rel: string, cwd = ROOT): View {
  return commitView(introducingCommitOf(rel, cwd), cwd)
}
let introducingCache: View | null = null
function introducingView(): View {
  if (introducingCache) return introducingCache
  introducingCache = introducingViewOf(PATHS.v106)
  return introducingCache
}
/** A subject is derived from the git objects of its candidate commit (SECTION_G3.package_subject). */
function deriveAtCandidate(cwd: string, kind: string, sha: string): Derived | string {
  if (!hasCommit(sha, cwd)) return 'candidate absent'
  const tree = text(cgit(cwd, ['rev-parse', `${sha}^{tree}`]))
  return deriveV3(C(), kind, sha, tree, commitView(sha, cwd))
}
...
function deriveV3(contract: Contract, kind: string, candidate: string, tree: string, view: View): Derived | string {
  ...
  if (entries.some((e) => /[\u0000-\u001f\u007f]/.test(e.path))) return UNENCODABLE
  const storeRoots = [contract.path_and_identity.adjudication_root, RETIRED_ROOT]
  if (entries.some((e) => storeRoots.some((r) => e.path.startsWith(r)))) return SELF_REF
...
const ADJUDICATION_CHECKS: CheckDef<Adjudication>[] = [
  ...
  { id: 'W7_DERIVED', when: 'both', check: (r, c) => {
    const d = c.derived
    if (d === SELF_REF) return 'STOP_SELF_REFERENTIAL_PACKAGE_CLOSURE'
...
const ATTEMPT_CHECKS: CheckDef<Attempt>[] = [
  ...
  { id: 'A6_SUBJECT', when: 'both', check: (r, c) => {
    ...
    for (const ref of [d, t] as (Identity | string | null)[]) {
      ...
      if (ref === SELF_REF) return 'STOP_SELF_REFERENTIAL_PACKAGE_CLOSURE'
  ...
  { id: 'A11_CANONICAL_CANDIDATE', when: 'both', check: (r) =>
    r.subject_kind !== PACKAGE || r.subject.candidate_sha === r.canonical_store.tip_observed_at_reservation ? null : 'STOP_ATTEMPT_SUBJECT_NOT_CANONICAL' },
  { id: 'A12_UNAMBIGUOUS', when: 'both', check: (r, c) =>
    c.existing.some((s) =>
      s.path !== c.recordPath &&
      s.record.subject_kind === r.subject_kind &&
      s.record.subject?.candidate_sha === r.subject.candidate_sha &&
      (s.record.subject.tree_sha !== r.subject.tree_sha || s.record.subject.package_digest !== r.subject.package_digest))
      ? 'STOP_PACKAGE_SUBJECT_AMBIGUOUS'
      : null },
  { id: 'R1_HISTORY', when: 'read', check: (_r, c) => (c.historyViolations.length === 0 ? null : 'STOP_ADJUDICATION_HISTORY_VIOLATED') },
  { id: 'R2_CHAIN', when: 'read', check: (r, c) => (chainIntegrity(kindRecords(c, r.subject_kind)) === null ? null : 'STOP_CHAIN_INTEGRITY_VIOLATED') },
...
interface MeasuredNow { provider_tip: string; rules_for_branch: { type: string; ruleset_id: number }[]; ruleset: ProviderRuleset & { id: number }; activity: { activity_type: string }[] }
const MEASURED_NOW = (C().canonical_store.provider_protection as unknown as { measured_2026_09_25: MeasuredNow }).measured_2026_09_25
/** The recorded 2026-09-25 provider state (ruleset 23975786 active), as evaluator input. */
const currentProvider = (): Provider => ({
  ref: CANONICAL_REF,
  tip: MEASURED_NOW.provider_tip,
  classic: null,
  rules: MEASURED_NOW.rules_for_branch.map((r) => ({ ...r })),
  rulesets: { [MEASURED_NOW.ruleset.id]: clone(MEASURED_NOW.ruleset) },
  activity: MEASURED_NOW.activity.map((a) => ({ activity_type: a.activity_type })),
})
...
const protectedProvider = (tip: string): Provider => {
  const rs = PRIOR.SECTION_F18_PROVIDER_BOUNDARY_REPORT.required_change.ruleset_json
  return {
    ref: CANONICAL_REF,
    tip,
    classic: null,
...
function consume(i: ConsumerInput): ConsumerResult {
  ...
  for (const s of i.store.records) {
    const ctx: Ctx = {
      mode: 'read',
      contract: i.contract,
      knownSubjects: PRIOR.SECTION_F5_KNOWN_SUBJECTS_V3.subjects,
  ...
  if (closure === SELF_REF) return { result: 'STOP_SELF_REFERENTIAL_PACKAGE_CLOSURE' }
...
const CONTRACT_BLOB = gitBlobSha(readBytes(PATHS.v106))
...
const contractRef = (): ContractRef => ({ path: PATHS.v106, section: C().record_schema.section, blob: CONTRACT_BLOB })
let attemptSeq = 0
...
const knownFor = (r: Rec): KnownSubject | undefined =>
  PRIOR.SECTION_F5_KNOWN_SUBJECTS_V3.subjects.find((k) => k.candidate_sha === r.subject.candidate_sha && k.subject_kind === r.subject_kind)
...
class FixtureRepo {
  ...
  storeAt(ref = 'HEAD', tip = ref, run: Runner = cgit): Store {
    ...
    const occ = PRIOR.SECTION_F7_MATERIALIZATION_AND_RESERVATION_WRITE_SET.retired_occurrence_root
...
/* ========================================================================== */
/* §1 owner v1.0.4, the effective contract and the lineage pins               */
/* ========================================================================== */

describe('§1 owner decisions, SECTION_G3 as F3 plus the declared delta, and the introducing-commit pins', () => {
  const owner = readJson<{ owner_decision_verbatim: Record<string, string[]>; structured_decision: Record<string, unknown>; companion_authority_amendment: string }>(PATHS.owner104)
  it('owner v1.0.4 verbatim and structured forms agree; SIGNED; Event A PENDING; blob pinned', () => {
    const d = owner.owner_decision_verbatim.OWNER_DECISION.join('\n')
    expect(d).toContain('CANONICAL_PROTECTED_CANDIDATE_BOUND')
    expect(d).toContain('PIN_TO_ACTUAL_INTRODUCING_COMMIT')
    expect(d).toContain('SIGNED = YES')
    expect(owner.owner_decision_verbatim.A_SUBJECT_MODEL.join('\n')).toContain('A prior PASS for subject A must never make subject B usable.')
    expect(owner.owner_decision_verbatim.B_INTRODUCING_VIEW.join('\n')).toContain('Pin the authority lineage mechanically.')
    expect(owner.structured_decision.RECOVERY_PACKAGE_SUBJECT_POLICY_V2).toBe('CANONICAL_PROTECTED_CANDIDATE_BOUND')
    expect(owner.structured_decision.INTRODUCING_VIEW_POLICY).toBe('PIN_TO_ACTUAL_INTRODUCING_COMMIT')
    expect(owner.structured_decision.PACKAGE_SUBJECT_TUPLE).toEqual(['candidate_sha', 'candidate_tree', 'package_digest'])
    expect(owner.structured_decision.EVENT_A_POLICY).toBe('PENDING')
    expect(owner.companion_authority_amendment).toBe(PATHS.v106)
    expect(AUTH.SECTION_G2_OWNER_DECISION_BINDING.owner_record_blob).toBe(gitBlobSha(readBytes(PATHS.owner104)))
  })
  it('SECTION_G3 equals v1.0.5 SECTION_F3 except at exactly the declared SECTION_G3_DELTA paths', () => {
    const diff: string[] = []
    const walk = (a: unknown, b: unknown, p: string): void => {
      if (a !== null && b !== null && typeof a === 'object' && typeof b === 'object' && !Array.isArray(a) && !Array.isArray(b)) {
        for (const k of new Set([...Object.keys(a), ...Object.keys(b)])) walk((a as Record<string, unknown>)[k], (b as Record<string, unknown>)[k], p ? `${p}.${k}` : k)
      } else if (canonical(a) !== canonical(b)) diff.push(p)
    }
    walk(PRIOR.SECTION_F3_CERTIFICATION_STORE_CONTRACT, C(), '')
    const declared = AUTH.SECTION_G3_DELTA.changed_paths
    const under = (d: string, p: string): boolean => d === p || d.startsWith(`${p}.`)
    expect(diff.filter((d) => !declared.some((p) => under(d, p)))).toEqual([])
    expect(declared.filter((p) => !diff.some((d) => under(d, p)))).toEqual([])
    expect(diff.length).toBeGreaterThan(0)
  })
  it('every introducing-commit pin is the unique adding commit (controlled read), with its tree', () => {
    const pins = AUTH.SECTION_G5_INTRODUCING_VIEW_PIN.pins
    expect(pins.length).toBe(16)
    for (const p of pins) {
      expect(introducingCommitOf(p.path), p.path).toBe(p.introducing_commit)
      expect(text(cgit(ROOT, ['rev-parse', `${p.introducing_commit}^{tree}`])), p.path).toBe(p.introducing_tree)
    }
    expect(pins.find((p) => p.path === PATHS.v105)).toMatchObject({ introducing_commit: 'ae958aa04f534be50a648bd1e43a069d0c2111c7', introducing_tree: '1bf379faaad626a6da6f1cd5a60c7afb3fa1defb' })
  })
  it('the v1.0.6 introducing view is the git commit that adds v1.0.6, not the worktree and not a patched base', () => {
    const c = introducingCommitOf(PATHS.v106)
  ...
  it('the delta is additive: no stop code, consumer result or unchanged validation step of F3 is removed', () => {
    ...
    for (const code of F.stop_codes) expect(C().stop_codes, code).toContain(code)
    for (const r of F.consumer_rule.consumer_results) expect(C().consumer_rule.consumer_results, r).toContain(r)
    for (const step of F.validation_order.filter((l) => !l.startsWith('W7 '))) expect(C().validation_order, step).toContain(step)
    for (const step of F.attempt_contract.validation_order.filter((l) => !l.startsWith('A6 '))) expect(C().attempt_contract.validation_order, step).toContain(step)
    expect(C().stop_codes.length).toBe(F.stop_codes.length + 3)
  })
})
...
describe('§2 RCO_CLOSURE_DIGEST_V3 and the package kind', () => {
  ...
  it('the package kind derives at the introducing tree: it covers this artifact, its owner record and this test, and excludes only absent references', () => {
    ...
    for (const p of [PATHS.v106, PATHS.owner104, PATHS.self, PATHS.battery, PATHS.manifest106, PATHS.v105, PATHS.v100]) expect(covered).toContain(p)
...
describe('§5 RECERT_ATTEMPT and ADJUDICATION validation', () => {
  ...
  it('a record citing a superseded contract (v1.0.4 SECTION_E3 or v1.0.5 SECTION_F3) is rejected', () => {
    for (const [p, sec] of [[PATHS.v104, 'SECTION_E3_ADJUDICATION_CONTRACT'], [PATHS.v105, 'SECTION_F3_CERTIFICATION_STORE_CONTRACT']]) {
      const r = clone(adj.record) as Adjudication
      r.contract = { path: p, section: sec, blob: gitBlobSha(readBytes(p)) }
      expect(validate(r, ctxFor(r, { existing: [att] })), p).toBe('STOP_ADJUDICATION_CONTRACT_MISMATCH')
    }
  })
...
describe('§7 canonical monotonicity, provider protection and store attacks', () => {
  ...
  it('a shallow clone of the store is UNVERIFIABLE', () => {
    ...
    expect(
      consume({
        ...
        isAncestor: (a, b) => clone2.isAncestor(a, b), contractPath: PATHS.v106, contractBlob: CONTRACT_BLOB, derivedFor: derivedDefault, targetFor: (r) => knownFor(r)?.id ?? null,
      }).result,
  ...
  it('a file under the retired occurrence root STOPS', () => {
    const s = new Scenario()
    s.adjudicate('PASS', s.attempt())
    s.repo.write(`${PRIOR.SECTION_F7_MATERIALIZATION_AND_RESERVATION_WRITE_SET.retired_occurrence_root}x.json`, '{}\n')
  ...
  it('a broken chain STOPS; a changed governing layer makes the certification STALE', () => {
    ...
    const added = patchedView(introducingView(), { 'docs/ops/release/STAGING_RECOVERY_EXECUTION_AUTHORITY_AMENDMENT_v1.0.7.json': { blob: '6'.repeat(40), text: '{}' } })
...
describe('§9 precedence anchors and the RECOVERY topic count', () => {
  ...
  const effectiveAnchors = (): Record<string, Anchor[]> => ({ ...clone(PRIOR.SECTION_F6_PRECEDENCE_ANCHORS.anchors), ...clone(AUTH.SECTION_G10_PRECEDENCE_ANCHORS.anchors_changed) })
  it('every topic is anchored to existing sections of files of its allocated domain (v1.0.5 anchors with SECTION_G10 changes)', () => {
    const anchors = effectiveAnchors()
    expect(AUTH.SECTION_G10_PRECEDENCE_ANCHORS.anchors_changed.RECOVERY_CERTIFICATION_OCCURRENCE_CONTRACT).toEqual([{ path: PATHS.v106, section: 'SECTION_G3_PACKAGE_SUBJECT_CONTRACT' }])
  ...
  it('an anchor moved to a file of another domain, to a missing section, or an empty anchor list is RED', () => {
    ...
    const missing = { ...clone(base), RECOVERY_CERTIFICATION_OCCURRENCE_CONTRACT: [{ path: PATHS.v106, section: 'SECTION_F99_ABSENT' }] }
  ...
  it('the allocation has 17 RECOVERY topics (not 22)', () => {
    ...
    expect(n).toBe(17)
    expect(PRIOR.SECTION_F6_PRECEDENCE_ANCHORS.recovery_topic_count).toBe(n)
  })
})
...
describe('§10 preservation, write set and the real repository', () => {
  it('every earlier artifact of the chain is byte-identical to its blob at the base and in the introducing tree', () => {
    for (const p of IMMUTABLE) {
      expect(commitView(BASE_COMMIT).blob(p), p).not.toBeNull()
      expect(introducingView().blob(p), p).toBe(commitView(BASE_COMMIT).blob(p))
      expect(gitBlobSha(storedBytes(p)), p).toBe(commitView(BASE_COMMIT).blob(p))
    }
  })
  it('the write set is exactly five added paths; the introducing tree holds no record and no occurrence', () => {
    ...
    expect(introducingView().list().filter((p) => p.startsWith(PRIOR.SECTION_F7_MATERIALIZATION_AND_RESERVATION_WRITE_SET.retired_occurrence_root))).toEqual([])
    expect(C().canonical_store.STORE_GENESIS.commit).toBe('c057ff71415f0545e95003321c88ea224f6c26f8')
    for (const p of WRITE_SET) expect(introducingView().blob(p), p).not.toBeNull()
  })
  it('declared layers are exactly the governing-family members added since each known candidate, with their blobs', () => {
    for (const ks of [KS_A, KS_B]) {
      ...
      const expected = introducingView()
        ...
        .sort(byteOrder)
        .map((p) => ({ path: p, blob: p === PATHS.v106 ? null : introducingView().blob(p) }))
  ...
  it('Event A is untouched; no operation added; the final state is ready for the next reservation', () => {
    expect(PRIOR.SECTION_F10_EVENT_A_PRESERVATION).toMatchObject({ CURRENT_EVENT_A_REACHABLE: 'NO', EVENT_A_POLICY: 'PENDING' })
    expect(AUTH.SECTION_G15_PRESERVATION.AUTHORIZED_OPERATIONS_DELTA).toEqual({ added: [], removed: [], modified: [] })
    expect(AUTH.final_state).toBe('RECOVERY_PACKAGE_SUBJECT_BOOTSTRAP_FIXED__RESERVATION_BLOCKED_BY_IMMUTABLE_V105_TEST_HEAD_STORE_ASSERTION')
    for (const l of AUTH.SECTION_G11_SUPERSEDED_LOCI) expect(l.chain).toBe('RECOVERY')
    expect(AUTH.SECTION_G4_CURRENT_AND_NEXT_SUBJECT.subject_at_lane_start).toMatchObject({ candidate_sha: BASE_COMMIT, candidate_tree: text(cgit(ROOT, ['rev-parse', `${BASE_COMMIT}^{tree}`])) })
  })
  ...
  it('the real repository: empty store at the introducing commit; at HEAD a live guard (whatever later lanes add must stay valid); the recorded provider states', { timeout: 900_000 }, () => {
    const root = C().path_and_identity.adjudication_root
    const intro = introducingCommitOf(PATHS.v106)
    expect(lsTree(ROOT, intro, [root])).toEqual([])
    expect(lsTree(ROOT, intro, [RETIRED_ROOT])).toEqual([])
    expect(everAddedViolations(ROOT, root, new Map(), cgit, intro)).toEqual([])
    expect(isAncestorIn(ROOT, BASE_COMMIT, 'HEAD')).toBe(true)
    const head = text(cgit(ROOT, ['rev-parse', 'HEAD']))
    const live = new FixtureRepo(ROOT).storeAt('HEAD', 'HEAD')
    expect(live.envViolations).toEqual([])
    expect(live.objectIntegrity).toEqual([])
    expect(live.shallow).toBe(false)
    ...
    const base: ConsumerInput = {
      ...
      isAncestor: (a, b) => isAncestorIn(ROOT, a, b), contractPath: PATHS.v106, contractBlob: CONTRACT_BLOB, derivedFor, targetFor: (r) => knownFor(r)?.id ?? null,
    }
    for (const kind of [CENSUS, OFFLINE, PACKAGE]) expect(consume({ ...base, kind }).result).toBe('STOP_PROVIDER_PROTECTION_REQUIRED')
    expect(protectionPass(currentProvider(), CANONICAL_REF)).toBe(true)
    expect(rewriteWitnessed(currentProvider())).toBe(false)
    const now = { ...currentProvider(), tip: head }
    const liveOk = ['USABLE', 'STOP_NOT_CERTIFIED', 'STOP_RECERT_ATTEMPT_OPEN', 'STOP_CERTIFICATION_REVOKED', 'STOP_CONTRADICTORY_CERTIFICATION', 'STOP_NOT_CURRENT', 'STOP_SUBJECT_NOT_PRESENT']
    for (const kind of [CENSUS, OFFLINE, PACKAGE]) expect(liveOk, kind).toContain(consume({ ...base, kind, provider: now }).result)
    const pkg = consume({ ...base, kind: PACKAGE, provider: now })
    expect(pkg.package_digest).toBe((deriveAtCandidate(ROOT, PACKAGE, head) as Derived).package_digest)
    if (live.raw.length === 0) expect(pkg).toMatchObject({ result: 'STOP_NOT_CERTIFIED', state: 'NO_ATTEMPT' })
  })
})
...
describe('§11 kill matrix over the record checks', () => {
  ...
  const NEG: Neg[] = [
    ...
    ((): Neg => {
      const r = attemptFor({ ...KS_B, id: 'PKG', subject_kind: PACKAGE }, [], { tip: 'e'.repeat(40) })
      return { id: 'a-noncanonical', record: r, ctx: ctxFor(r), want: 'STOP_ATTEMPT_SUBJECT_NOT_CANONICAL' }
    })(),
    ((): Neg => {
      const other = attemptFor(KS_A, [], { tip: 'e'.repeat(40), attemptId: 'KM-OTHER-TREE' })
      other.subject.tree_sha = 'c'.repeat(40)
      return { id: 'a-ambiguous', record: att.record, ctx: ctxFor(att.record, { existing: [stored(other)] }), want: 'STOP_PACKAGE_SUBJECT_AMBIGUOUS' }
    })(),
    negA('self-ref', 'STOP_SELF_REFERENTIAL_PACKAGE_CLOSURE', () => undefined, { target: null, derived: SELF_REF }),
    negT('a-self-ref', 'STOP_SELF_REFERENTIAL_PACKAGE_CLOSURE', () => undefined, { target: null, derived: SELF_REF }),
...
/* ========================================================================== */
/* §12 package subject bound to the protected canonical candidate (owner v1.0.4) */
/* ========================================================================== */

const PKG_AUTH = 'docs/ops/release/STAGING_RECOVERY_EXECUTION_AUTHORITY_AMENDMENT_v9.9.1.json'
const PKG_TEST = 'tests/release/staging-recovery-fixture.test.ts'
/** A repository holding a complete Recovery package (covered files and toolchain roots). */
class PackageRepo extends FixtureRepo {
  readonly introducing: string
  constructor() {
    super()
    for (const t of C().subject_kinds.kinds[PACKAGE].toolchain_roots) this.write(t, t === 'vitest.config.ts' ? 'export default {}\n' : `{"fixture":"${t}"}\n`)
    this.write(PKG_AUTH, '{"version":"9.9.1"}\n')
    this.write(PKG_TEST, 'export const fixture = 1\n')
    this.introducing = this.commit('introduce the package authority')
  }
  subject(sha = this.head()): Derived {
    const d = deriveAtCandidate(this.dir, PACKAGE, sha)
    if (typeof d === 'string') throw new Error(d)
    return d
  }
  coveredChange(n: number): string {
    this.write(PKG_TEST, `export const fixture = ${n}\n`)
    return this.commit(`covered change ${n}`)
  }
}
const asSubject = (d: Identity): KnownSubject => ({ ...clone(d), id: 'PKG', subject_kind: PACKAGE, adjudication_directory: '' })
/** SECTION_G3.attempt_contract.reservation_authorization C6: the record never changes its subject's closure. */
function closureUnchangedByRecord(cwd: string, candidate: string, recordPath: string, recordText: string): boolean {
  const before = deriveAtCandidate(cwd, PACKAGE, candidate)
  const view = patchedView(commitView(candidate, cwd), { [recordPath]: { blob: gitBlobSha(Buffer.from(recordText, 'utf8')), text: recordText } })
  const after = deriveV3(C(), PACKAGE, candidate, text(cgit(cwd, ['rev-parse', `${candidate}^{tree}`])), view)
  return typeof before !== 'string' && typeof after !== 'string' && before.package_digest === after.package_digest
}
/** SECTION_G3.attempt_contract.reservation_authorization C1..C6 at the reservation prestate. */
function reservationStop(cwd: string, provider: Provider, s: Stored, existing: Stored[]): string | null {
  const r = s.record as Attempt
  if (r.subject_kind === PACKAGE && (provider.ref !== CANONICAL_REF || provider.tip !== r.subject.candidate_sha || r.canonical_store.tip_observed_at_reservation !== provider.tip)) return 'STOP_ATTEMPT_SUBJECT_NOT_CANONICAL'
  if (!protectionPass(provider, CANONICAL_REF)) return 'STOP_PROVIDER_PROTECTION_REQUIRED'
  if (rewriteWitnessed(provider)) return 'STOP_CANONICAL_STORE_REWRITTEN'
  const v = validate(r, ctxFor(r, { existing, recordPath: s.path, derived: deriveAtCandidate(cwd, r.subject_kind, r.subject.candidate_sha), target: knownFor(r)?.id ?? null }))
  if (v !== 'VALID') return v
  if (!closureUnchangedByRecord(cwd, r.subject.candidate_sha, s.path, canonicalPretty(r))) return 'STOP_SELF_REFERENTIAL_PACKAGE_CLOSURE'
  return null
}
class PackageScenario {
  readonly repo = new PackageRepo()
  readonly recs: Stored[] = []
  ks(sha = this.repo.head()): KnownSubject {
    return asSubject(this.repo.subject(sha))
  }
  attempt(over: AttemptOver = {}, subject: KnownSubject = this.ks()): Stored {
    const s = stored(attemptFor(subject, this.recs, { tip: subject.candidate_sha, ...over }))
    this.repo.put(s)
    this.repo.commit('reserve a package attempt')
    this.recs.push(s)
    return s
  }
  adjudicate(cls: string, att: Stored, edit?: (r: Adjudication) => void): Stored {
    const r = adjudicationFor(this.ks((att.record as Attempt).subject.candidate_sha), cls, att, this.recs)
    edit?.(r)
    const s = stored(r)
    this.repo.put(s)
    this.repo.commit(`materialize ${cls}`)
    this.recs.push(s)
    return s
  }
  consume(over: Partial<ConsumerInput> = {}): ConsumerResult {
    const head = this.repo.head()
    return consume({
      contract: C(),
      kind: PACKAGE,
      head: commitView(head, this.repo.dir),
      headSha: head,
      store: this.repo.storeAt('HEAD', head),
      provider: protectedProvider(head),
      genesis: this.repo.genesis,
      ...
      contractPath: PATHS.v106,
      contractBlob: CONTRACT_BLOB,
      derivedFor: (r) => (r.subject_kind === PACKAGE ? deriveAtCandidate(this.repo.dir, PACKAGE, r.subject.candidate_sha) : derivedDefault(r)),
      targetFor: (r) => knownFor(r)?.id ?? null,
      ...over,
    })
  }
...
describe('§12 package subject: the protected canonical candidate, its own tree, its own digest', () => {
  it('H1 the introducing amendment commit vs a later covered test fix: distinct subjects; only the protected tip is reservable', () => {
    const s = new PackageScenario()
    const I = s.repo.introducing
    const F = s.repo.coveredChange(2)
    expect(introducingCommitOf(PKG_AUTH, s.repo.dir)).toBe(I)
    expect(introducingCommitOf(PKG_TEST, s.repo.dir)).toBe(I)
    expect(s.repo.subject(I).package_digest).not.toBe(s.repo.subject(F).package_digest)
    const provider = protectedProvider(F)
    expect(reservationStop(s.repo.dir, provider, stored(attemptFor(s.ks(F), [], { tip: F })), [])).toBeNull()
    expect(reservationStop(s.repo.dir, provider, stored(attemptFor(s.ks(I), [], { tip: I })), [])).toBe('STOP_ATTEMPT_SUBJECT_NOT_CANONICAL')
  })
  it('H2 a PASS for digest A is not reused after a covered change produces digest B (NO_ATTEMPT)', () => {
    const s = new PackageScenario()
    const A = s.repo.subject()
    s.adjudicate('PASS', s.attempt())
    expect(s.consume()).toMatchObject({ result: 'USABLE', state: 'RESOLVED_PASS', package_digest: A.package_digest })
    s.repo.coveredChange(2)
    const r = s.consume()
    expect(r).toMatchObject({ result: 'STOP_NOT_CERTIFIED', state: 'NO_ATTEMPT' })
    expect(r.package_digest).not.toBe(A.package_digest)
  })
  it('H3/H5/H6 a subject whose tree or digest was computed from another head, or is simply wrong, is rejected', () => {
    const s = new PackageScenario()
    const A = s.repo.head()
    const M = s.repo.coveredChange(2)
    s.repo.g(['checkout', '-q', A])
    const provider = protectedProvider(A)
    const fromHead = stored(attemptFor({ ...s.ks(M), candidate_sha: A }, [], { tip: A }))
    expect(reservationStop(s.repo.dir, provider, fromHead, [])).toBe('STOP_TREE_SHA_MISMATCH')
    const wrongTree = stored(attemptFor({ ...s.ks(A), tree_sha: 'c'.repeat(40) }, [], { tip: A }))
    expect(reservationStop(s.repo.dir, provider, wrongTree, [])).toBe('STOP_TREE_SHA_MISMATCH')
    const wrongDigest = stored(attemptFor({ ...s.ks(A), package_digest: 'f'.repeat(64) }, [], { tip: A }))
    expect(reservationStop(s.repo.dir, provider, wrongDigest, [])).toBe('STOP_PACKAGE_DIGEST_MISMATCH')
    const good = stored(attemptFor(s.ks(A), [], { tip: A }))
    expect(reservationStop(s.repo.dir, provider, good, [])).toBeNull()
  })
  it('H3 at read time: an attempt whose digest was recomputed from the current head makes the store unusable', () => {
    const s = new PackageScenario()
    const A = s.repo.head()
    const M = s.repo.coveredChange(2)
    s.attempt({ tip: A }, { ...s.ks(M), candidate_sha: A })
    expect(s.consume().result).toBe('STOP_ADJUDICATION_INVALID_AT_READ')
  })
  it('H4 a reservation bound to a stale or noncanonical candidate STOPS (write time and read time)', () => {
    const s = new PackageScenario()
    const A = s.repo.head()
    const M = s.repo.coveredChange(2)
    expect(reservationStop(s.repo.dir, protectedProvider(M), stored(attemptFor(s.ks(A), [], { tip: A })), [])).toBe('STOP_ATTEMPT_SUBJECT_NOT_CANONICAL')
    expect(reservationStop(s.repo.dir, protectedProvider(M), stored(attemptFor(s.ks(A), [], { tip: M })), [])).toBe('STOP_ATTEMPT_SUBJECT_NOT_CANONICAL')
    const stale = new PackageScenario()
    const A2 = stale.repo.head()
    const M2 = stale.repo.coveredChange(2)
    stale.attempt({ tip: M2 }, stale.ks(A2))
    expect(stale.consume().result).toBe('STOP_ADJUDICATION_INVALID_AT_READ')
    const side = new PackageScenario()
    side.repo.g(['checkout', '-q', '-b', 'side'])
    const S = side.repo.coveredChange(3)
    const sideSubject = side.ks(S)
    side.repo.g(['checkout', '-q', 'main'])
    side.attempt({ tip: S }, sideSubject)
    expect(side.repo.isAncestor(S, side.repo.head())).toBe(false)
    expect(side.consume().result).toBe('STOP_CANONICAL_STORE_NOT_MONOTONIC')
  })
  it('H7 a result that resolves the attempt of another subject is rejected', () => {
    const s = new PackageScenario()
    const A = s.repo.head()
    const att = s.attempt()
    const M = s.repo.coveredChange(2)
    const B = s.ks(M)
    s.adjudicate('PASS', att, (r) => {
      r.subject = { candidate_sha: B.candidate_sha, tree_sha: B.tree_sha, digest_algorithm: B.digest_algorithm, entries: clone(B.entries), package_digest: B.package_digest, governing_families: [...B.governing_families], governing_family_members_at_candidate: clone(B.governing_family_members_at_candidate) }
      r.provenance.independent_adjudicator.examined_candidate_sha = B.candidate_sha
      r.provenance.independent_adjudicator.examined_tree_sha = B.tree_sha
    })
    expect(s.consume().result).toBe('STOP_ADJUDICATION_INVALID_AT_READ')
    expect(A).not.toBe(M)
  })
  it('H8/H9 reservation and outcome commits move the head without moving the subject', () => {
    const s = new PackageScenario()
    const A = s.repo.subject()
    const att = s.attempt()
    expect(s.repo.head()).not.toBe(A.candidate_sha)
    const open = s.consume()
    expect(open).toMatchObject({ result: 'STOP_RECERT_ATTEMPT_OPEN', state: 'OPEN_ATTEMPT', package_digest: A.package_digest })
    expect((att.record as Attempt).subject).toEqual({ candidate_sha: A.candidate_sha, tree_sha: A.tree_sha, package_digest: A.package_digest, digest_algorithm: A.digest_algorithm })
    s.adjudicate('PASS_WITH_NONBLOCKING_FINDINGS', att)
    expect(s.consume()).toMatchObject({ result: 'USABLE', state: 'RESOLVED_PASS', package_digest: A.package_digest })
  })
  it('H10 a live-disk mutation never changes the introducing view; the v1.0.5 live-bytes construction would', () => {
    const s = new PackageScenario()
    const I = s.repo.introducing
    const before = s.repo.subject(I).package_digest
    writeFileSync(path.join(s.repo.dir, PKG_AUTH), '{"version":"tampered on disk"}\n')
    const viaGit = introducingViewOf(PKG_AUTH, s.repo.dir)
    const tree = text(cgit(s.repo.dir, ['rev-parse', `${I}^{tree}`]))
    expect((deriveV3(C(), PACKAGE, I, tree, viaGit) as Derived).package_digest).toBe(before)
    const liveBytes = readFileSync(path.join(s.repo.dir, PKG_AUTH))
    const live = patchedView(commitView(I, s.repo.dir), { [PKG_AUTH]: { blob: gitBlobSha(liveBytes), text: liveBytes.toString('utf8') } })
    expect((deriveV3(C(), PACKAGE, I, tree, live) as Derived).package_digest).not.toBe(before)
  })
  it('H11 a rewrite witness or a force-pushed canonical history STOPS reservation and consumption', () => {
    const s = new PackageScenario()
    const A = s.repo.head()
    const witnessed = protectedProvider(A)
    witnessed.activity.push({ activity_type: 'force_push' })
    expect(reservationStop(s.repo.dir, witnessed, stored(attemptFor(s.ks(A), [], { tip: A })), [])).toBe('STOP_CANONICAL_STORE_REWRITTEN')
    expect(reservationStop(s.repo.dir, { ...protectedProvider(A), rules: [] }, stored(attemptFor(s.ks(A), [], { tip: A })), [])).toBe('STOP_PROVIDER_PROTECTION_REQUIRED')
    s.adjudicate('PASS', s.attempt())
    const p = protectedProvider(s.repo.head())
    p.activity.push({ activity_type: 'force_push' })
    expect(s.consume({ provider: p }).result).toBe('STOP_CANONICAL_STORE_REWRITTEN')
    const rewritten = s.repo.g(['commit-tree', `${s.repo.head()}^{tree}`, '-p', s.repo.genesis, '-m', 'rewritten canonical'])
    s.repo.g(['reset', '-q', '--hard', rewritten])
    expect(s.consume().result).toBe('STOP_CANONICAL_STORE_NOT_MONOTONIC')
  })
  it('H12 a deleted historical record (attempt or outcome) makes the store unusable', () => {
    const s = new PackageScenario()
    const att = s.attempt()
    const out = s.adjudicate('PASS', att)
    s.repo.g(['rm', '-q', out.path])
    s.repo.commit('delete the outcome')
    expect(s.consume().result).toBe('STOP_ADJUDICATION_HISTORY_VIOLATED')
  })
  it('H13 an open attempt still suspends an earlier PASS of the same subject', () => {
    const s = new PackageScenario()
    s.adjudicate('PASS', s.attempt())
    expect(s.consume().result).toBe('USABLE')
    s.attempt()
  ...
  it('F the package closure excludes the store mechanically; a closure reaching a store path STOPS', () => {
    const prefixes = C().subject_kinds.kinds[PACKAGE].covered_path_prefixes
    for (const root of [C().path_and_identity.adjudication_root, RETIRED_ROOT]) expect(prefixes.some((p) => root.startsWith(p) || p.startsWith(root)), root).toBe(false)
    const real = deriveV3(C(), PACKAGE, '0'.repeat(40), '1'.repeat(40), introducingView())
    expect(typeof real).not.toBe('string')
    const recordPath = `${C().path_and_identity.adjudication_root}PACKAGE/${(real as Derived).package_digest}/a-${'1'.repeat(64)}.json`
    const withRecord = patchedView(introducingView(), { [recordPath]: { blob: '9'.repeat(40), text: '{}\n' } })
    expect((deriveV3(C(), PACKAGE, '0'.repeat(40), '1'.repeat(40), withRecord) as Derived).package_digest).toBe((real as Derived).package_digest)
    const referencing = patchedView(withRecord, { 'tests/release/staging-recovery-x.test.ts': { blob: '8'.repeat(40), text: `// ${recordPath}\n` } })
    expect(deriveV3(C(), PACKAGE, '0'.repeat(40), '1'.repeat(40), referencing)).toBe(SELF_REF)
    const s = new PackageScenario()
    const A = s.repo.head()
    const att = stored(attemptFor(s.ks(A), [], { tip: A }))
    expect(closureUnchangedByRecord(s.repo.dir, A, att.path, canonicalPretty(att.record))).toBe(true)
    s.repo.write(PKG_TEST, `// ${att.path}\n`)
    const R = s.repo.commit('a covered file names a concrete record path')
    expect(closureUnchangedByRecord(s.repo.dir, R, att.path, canonicalPretty(att.record))).toBe(false)
    s.repo.write(att.path, '{}\n')
    s.repo.commit('the named record path now exists')
    expect(deriveAtCandidate(s.repo.dir, PACKAGE, s.repo.head())).toBe(SELF_REF)
  })
  it('N-1 a consumer head whose closure reaches an existing record path is STOP_SELF_REFERENTIAL_PACKAGE_CLOSURE', () => {
    const s = new PackageScenario()
    const att = s.attempt()
    s.repo.write(PKG_TEST, `// ${att.path}\n`)
    s.repo.commit('a covered file names the existing attempt path')
    expect(s.consume().result).toBe('STOP_SELF_REFERENTIAL_PACKAGE_CLOSURE')
  })
  it('B-2 a path containing TAB or LF could forge the digest encoding; such a path fails the derivation', () => {
    const naive = (entries: Entry[]): string => digestOf(entries)
    const X = 'a'.repeat(40)
    const Y = 'b'.repeat(40)
    const two: Entry[] = [{ role: 'COVERED', path: 'p1', blob: X }, { role: 'COVERED', path: 'p2', blob: Y }]
    const one: Entry[] = [{ role: 'COVERED', path: `p1\t${X}\nCOVERED\tp2`, blob: Y }]
    expect(naive(one)).toBe(naive(two))
    for (const bad of ['tests/release/staging-recovery-a\tb.test.ts', 'tests/release/staging-recovery-a\nb.test.ts', 'tests/release/staging-recovery-a\u007fb.test.ts']) {
      const v = patchedView(introducingView(), { [bad]: { blob: '7'.repeat(40), text: '\n' } })
      expect(deriveV3(C(), PACKAGE, '0'.repeat(40), '1'.repeat(40), v), JSON.stringify(bad)).toBe(UNENCODABLE)
    }
    const repo = new PackageRepo()
    const raw = (args: string[], input?: string): string => execFileSync('git', args, { cwd: repo.dir, input, encoding: 'utf8', env: { ...process.env, GIT_CEILING_DIRECTORIES: path.dirname(repo.dir) } })
    const oid = raw(['hash-object', '-w', '--stdin'], 'x\n').trim()
    const bad = 'tests/release/staging-recovery-a\tb\nc.test.ts'
    const parts = bad.split('/')
    /** git plumbing accepts a TAB/LF name that the Windows index refuses: build the trees bottom-up with mktree -z. */
    const build = (treeish: string | null, depth: number): string => {
      const entries = treeish ? raw(['ls-tree', '-z', treeish]).split('\0').filter(Boolean) : []
      const name = parts[depth]
      const nameOf = (e: string): string => e.slice(e.indexOf('\t') + 1)
      let line: string
      if (depth === parts.length - 1) line = `100644 blob ${oid}\t${name}`
      else {
        const existing = entries.find((e) => nameOf(e) === name)
        line = `040000 tree ${build(existing ? existing.slice(0, existing.indexOf('\t')).split(' ')[2] : null, depth + 1)}\t${name}`
      }
      return raw(['mktree', '-z'], [...entries.filter((e) => nameOf(e) !== name), line].map((e) => `${e}\0`).join('')).trim()
    }
    const tree = build(`${repo.head()}^{tree}`, 0)
    const commit = raw(['-c', 'user.name=fixture', '-c', 'user.email=fixture@example.invalid', 'commit-tree', tree, '-p', repo.head(), '-m', 'a covered path containing TAB and LF'], '').trim()
    expect(objectIntegrityViolations(repo.dir)).toEqual([])
    expect(lsTree(repo.dir, commit, ['tests/release/']).map((e) => e.path)).toContain(bad)
    expect(deriveAtCandidate(repo.dir, PACKAGE, commit)).toBe(UNENCODABLE)
  })
  it('N-4 the introducing commit is unverifiable in a shallow clone and ambiguous for a path added twice', () => {
    const s = new PackageScenario()
    s.repo.coveredChange(2)
    const dst = path.join(PRIVATE, `shallow${repoSeq++}`)
    execFileSync('git', ['clone', '-q', '--depth', '1', `file:///${s.repo.dir.replace(/\\/g, '/')}`, dst], { stdio: 'ignore' })
    expect(() => introducingCommitOf(PKG_AUTH, dst)).toThrow(/shallow/)
    s.repo.g(['rm', '-q', PKG_AUTH])
    s.repo.commit('delete the authority')
    s.repo.write(PKG_AUTH, '{"version":"9.9.1 again"}\n')
    s.repo.commit('re-add the authority')
    expect(() => introducingCommitOf(PKG_AUTH, s.repo.dir)).toThrow(/2 candidates/)
  })
  it('C1 a provider observation for another ref does not authorize a reservation', () => {
    const s = new PackageScenario()
    const A = s.repo.head()
    expect(reservationStop(s.repo.dir, { ...protectedProvider(A), ref: 'refs/heads/main' }, stored(attemptFor(s.ks(A), [], { tip: A })), [])).toBe('STOP_ATTEMPT_SUBJECT_NOT_CANONICAL')
  })
  it('E a result claiming subject A with a tree or digest that does not match its candidate is rejected', () => {
    const s = new PackageScenario()
    const att = s.attempt()
    s.adjudicate('PASS', att, (r) => {
      r.subject.package_digest = 'f'.repeat(64)
    })
    expect(s.consume().result).toBe('STOP_ADJUDICATION_INVALID_AT_READ')
  })
  it('the SECTION_G6 cases are the owner brief E cases, each resolved to a declared code or state', () => {
    const cases = AUTH.SECTION_G6_CONSUMER_SEMANTICS.cases
    expect(cases.length).toBe(6)
    const owner = readJson<{ owner_decision_verbatim: Record<string, string[]> }>(PATHS.owner104).owner_decision_verbatim.E_CONSUMER_SEMANTICS
