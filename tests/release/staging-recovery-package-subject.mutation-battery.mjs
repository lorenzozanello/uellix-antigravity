#!/usr/bin/env node
// tests/release/staging-recovery-package-subject.mutation-battery.mjs
//
// Independent mutation battery for the Recovery PACKAGE SUBJECT interpreter
// (tests/release/staging-recovery-package-subject.test.ts, authority
// STAGING_RECOVERY_EXECUTION_AUTHORITY_AMENDMENT_v1.0.6). It carries every
// v1.0.5 mutant of the shared interpreter (re-run against this copy) except the
// registry ones (the registry is unchanged and not in this file), and adds the
// package-subject, introducing-view, reservation and self-referential-closure
// mutants.
//
...
//
// Run:   node tests/release/staging-recovery-package-subject.mutation-battery.mjs
// Check: node tests/release/staging-recovery-package-subject.mutation-battery.mjs --check-anchors
// Subset: RPS_BATTERY_ONLY=PS1,PS2,HARMLESS node ... (reported as BATTERY_SUBSET_*, never as a full pass)
...
const TARGET = 'tests/release/staging-recovery-package-subject.test.ts'
const VITEST = path.join(ROOT, 'node_modules', 'vitest', 'vitest.mjs')
const PREFIX = 'zz-rps-mutant-'
const CONCURRENCY = Number(process.env.RPS_BATTERY_CONCURRENCY ?? 5)
...
// - reservation C6 inside reservationStop: for a well-formed record, a closure file
//   would have to name rca/PACKAGE/<P>/a-<D>.json where P depends on that same file
//   (a sha256 fixed point), so C6 cannot fire after C4/C5; the closure comparison it
//   uses (closureUnchangedByRecord) is mutated as PS15 and tested directly;
...
export const EQUIVALENT = [
  ...
  'reservation C6 inside reservationStop (unreachable for a well-formed record: a sha256 fixed point; the closure comparison itself is mutated as PS15)',
]
...
export const MUTANTS = [
  ...
  // ---- package subject, introducing view, reservation, self-referential closure (v1.0.6)
  { id: 'PS1', cls: 'SUBJECT', why: 'a closure reaching a store path is accepted', anchor: "  if (entries.some((e) => storeRoots.some((r) => e.path.startsWith(r)))) return SELF_REF", replacement: '' },
  { id: 'PS2', cls: 'SUBJECT', why: 'W7 maps a self-referential closure to a generic mismatch', anchor: "    if (d === SELF_REF) return 'STOP_SELF_REFERENTIAL_PACKAGE_CLOSURE'\n", replacement: '' },
  { id: 'PS3', cls: 'SUBJECT', why: 'A6 maps a self-referential closure to a generic mismatch', anchor: "      if (ref === SELF_REF) return 'STOP_SELF_REFERENTIAL_PACKAGE_CLOSURE'\n", replacement: '' },
  { id: 'PS4', cls: 'SUBJECT', why: 'consumer maps a self-referential closure to STOP_NOT_CURRENT', anchor: "  if (closure === SELF_REF) return { result: 'STOP_SELF_REFERENTIAL_PACKAGE_CLOSURE' }\n", replacement: '' },
  { id: 'PS5', cls: 'SUBJECT', why: 'a package attempt for a candidate other than the observed tip is accepted (A11)', anchor: "    r.subject_kind !== PACKAGE || r.subject.candidate_sha === r.canonical_store.tip_observed_at_reservation ? null : 'STOP_ATTEMPT_SUBJECT_NOT_CANONICAL' },", replacement: '    null },' },
  { id: 'PS6', cls: 'SUBJECT', why: 'an ambiguously represented subject is accepted (A12)', anchor: "      ? 'STOP_PACKAGE_SUBJECT_AMBIGUOUS'\n", replacement: '      ? null\n' },
  { id: 'PS7', cls: 'INTRODUCING', why: 'the introducing commit is any commit touching the path, not the one adding it', anchor: "['log', '--full-history', '--diff-filter=A', '--format=%H', ref, '--',", replacement: "['log', '--full-history', '--format=%H', ref, '--'," },
  { id: 'PS8', cls: 'INTRODUCING', why: 'the introducing view reads live worktree bytes (the v1.0.5 defect)', anchor: '  return commitView(introducingCommitOf(rel, cwd), cwd)\n', replacement: "  return patchedView(commitView(introducingCommitOf(rel, cwd), cwd), { [rel]: { blob: gitBlobSha(readFileSync(path.join(cwd, rel))), text: readFileSync(path.join(cwd, rel), 'utf8') } })\n" },
  { id: 'PS9', cls: 'SUBJECT', why: 'the subject is derived from the head that reads it, not from its candidate', anchor: '  return deriveV3(C(), kind, sha, tree, commitView(sha, cwd))\n', replacement: "  return deriveV3(C(), kind, sha, tree, commitView('HEAD', cwd))\n" },
  { id: 'PS10', cls: 'SUBJECT', why: 'the subject tree is read from the head, not from the candidate', anchor: "  const tree = text(cgit(cwd, ['rev-parse', `${sha}^{tree}`]))\n  return deriveV3", replacement: "  const tree = text(cgit(cwd, ['rev-parse', 'HEAD^{tree}']))\n  return deriveV3" },
  { id: 'PS11', cls: 'RESERVATION', why: 'reservation not bound to the protected tip of the canonical ref (C1)', anchor: "  if (r.subject_kind === PACKAGE && (provider.ref !== CANONICAL_REF || provider.tip !== r.subject.candidate_sha || r.canonical_store.tip_observed_at_reservation !== provider.tip)) return 'STOP_ATTEMPT_SUBJECT_NOT_CANONICAL'\n", replacement: '' },
  { id: 'PS12', cls: 'RESERVATION', why: 'reservation without provider protection (C2)', anchor: "  if (!protectionPass(provider, CANONICAL_REF)) return 'STOP_PROVIDER_PROTECTION_REQUIRED'\n  if (rewriteWitnessed(provider)) return 'STOP_CANONICAL_STORE_REWRITTEN'\n  const v = validate(", replacement: "  if (rewriteWitnessed(provider)) return 'STOP_CANONICAL_STORE_REWRITTEN'\n  const v = validate(" },
  { id: 'PS13', cls: 'RESERVATION', why: 'reservation after a rewrite witness (C3)', anchor: "  if (rewriteWitnessed(provider)) return 'STOP_CANONICAL_STORE_REWRITTEN'\n  const v = validate(", replacement: '  const v = validate(' },
  { id: 'PS19', cls: 'RESERVATION', why: 'C1 ignores the provider-reported ref (review N-6)', anchor: 'provider.ref !== CANONICAL_REF || provider.tip !== r.subject.candidate_sha', replacement: 'provider.tip !== r.subject.candidate_sha' },
  { id: 'PS16', cls: 'SUBJECT', why: 'a path with TAB/LF/control characters is digested (ambiguous S8 encoding, review B-2)', anchor: '  if (entries.some((e) => /[\\u0000-\\u001f\\u007f]/.test(e.path))) return UNENCODABLE\n', replacement: '' },
...
async function main() {
  ...
  const only = (process.env.RPS_BATTERY_ONLY ?? '').split(',').filter(Boolean)
