...
async function main(): Promise<void> {
  ...
  const result = await runGuardedContextualEvaluation({ cases: OFFICIAL_CONTEXTUAL_MOCK_CASES, caseIds: activeCaseIds, dryRun: args.dryRun, env: process.env, runtime: currentRuntime, provider })
  if (args.dryRun) { console.log(JSON.stringify(result.summary)); return }
  const output = resumed ? { directory: resolve(args.resume!), runId: String(resumed.manifest.runId) } : await createRunArtifacts(resolve(args.outputRoot ?? 'artifacts/stella-contextual-real-runs'), args.runLabel)
