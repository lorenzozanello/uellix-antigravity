...
// RED), OR-N17 (artifact inside the repository -> RED), TOC structure.
...
import { checkArchiveStructure, checkArtifactLocation, isInsideDirectory, parseArchiveToc, verifyArtifactDigest } from '../../scripts/recovery/artifact-integrity'
...
function artifact(bytes: Buffer) {
  const p = path.join(dir, `a-${createHash('sha256').update(bytes).digest('hex').slice(0, 8)}.dump`)
...
describe('artifact digest (layer 2)', () => {
  it('accepts bytes whose recomputed sha256 and size match the packet', async () => {
  ...
  it('OR-N2: truncation is a SIZE mismatch; absence is ABSENT', async () => {
    const { p, packet } = artifact(Buffer.from('PGDMP-synthetic-bytes-4'))
    writeFileSync(p, Buffer.from('PGDMP'))
    expect(await verifyArtifactDigest(packet, p, REPO)).toEqual({ ok: false, code: 'ARTIFACT_SIZE_MISMATCH' })
...
describe('archive structure (layer 3)', () => {
  ...
  it('OR-N1: TABLE entries must equal the captured relations exactly', () => {
    const packet = samplePacket()
    expect(checkArchiveStructure(parseArchiveToc(LISTING), packet)).toBeNull()
    const missing = LISTING.replace(/^222; .*$/m, '')
    expect(checkArchiveStructure(parseArchiveToc(missing), packet)).toEqual({ ok: false, code: 'ARTIFACT_TOC_RELATIONS_MISMATCH' })
  })

  it('OR-N1: a non-custom or empty archive is refused', () => {
    const packet = samplePacket()
    expect(checkArchiveStructure(parseArchiveToc(LISTING.replace('Format: CUSTOM', 'Format: TAR')), packet)).toEqual({ ok: false, code: 'ARTIFACT_TOC_NOT_CUSTOM_FORMAT' })
    expect(checkArchiveStructure(parseArchiveToc(';\n;     Format: CUSTOM\n'), packet)).toEqual({ ok: false, code: 'ARTIFACT_TOC_EMPTY' })
  })
})
