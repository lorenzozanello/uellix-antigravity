...
describe('dn0ConfigFor (the real entry-point configuration)', () => {
  ...
  it('the b25f2d32 delta recert is historical only: it can never certify a new candidate (v1.0.8)', () => {
    const b25 = cfg.certificationEvents[4]
    const raw = readFileSync(b25.path, 'utf8')
    expect(assessCertificationEvent(raw, b25).ok).toBe(true)
