...
describe('E2/CTRL-M3: immutableByConvention closed-world guard', () => {
  it('is the exact 46-member closed world (20 pinned entries + v1.0.10 + v1.0.11 + v1.0.12 + v1.0.13 + v1.0.14 + v1.0.15 + v1.0.16 + v1.0.17 + v1.0.18 + v1.0.19 + v1.0.20 + v1.0.21 + v1.0.22 + v1.0.23 + v1.0.24 + v1.0.25 + v1.0.26 + v1.0.27 + v1.0.28 + v1.0.29 + v1.0.30 + v1.0.31 + v1.0.32 + v1.0.33 + v1.0.34 + v1.0.35)', () => {
    expect(IMMUTABLE_BY_CONVENTION.length).toBe(46)
    expect(new Set(IMMUTABLE_BY_CONVENTION).size).toBe(46)
  ...
  // CONTROLLER_42_RULE, v1.0.32.json, v1.0.33.json and v1.0.34.json
  // CONTROLLER_SUCCESSOR_RULE and now v1.0.35.json CONTROLLER_SUCCESSOR_RULE):
  // the closed world does not pre-include a later ODS successor artifact. The
  // control ADVANCES with the list — v1.0.35 is now enumerated by this
  // successor act, so the absence assertion moves to the mechanically-next
  // version. This is exactly the act v1.0.35.json
  // CONTROLLER_SUCCESSOR_RULE.requirement names in its own words:
  // "The NEXT Controller successor act is REQUIRED to enumerate docs/ops/ods/
  // ODS_V1_MAINTENANCE_ADDENDUM_v1.0.35.json in IMMUTABLE_BY_CONVENTION after
  // this artifact is integrated, taking the array from its then-live length to
  // that length PLUS EXACTLY ONE, with v1.0.35 as the FINAL item, and advancing
  // the absence-control literal from v1.0.35 to the mechanically-next lineage
  ...
  // v1.0.35 CONTROLLER_SUCCESSOR_RULE.NO_CONTROLLER_ORDINAL_RESERVED is binding
  ...
  // that the 45 it measured "is a MEASUREMENT of the present, not a reservation
  // of a future label". Measured here at BASE
  // eafaaf57b0ad01031d9c61a985fbc6a2da817340 by structural bracket matching of
  // scripts/ods-controller.ts — 45 entries, 45 unique, final entry v1.0.34 —
  // which is what makes 46 below a derived figure rather than a remembered one.
  //
  // NO ORDINAL LABEL IS USED FOR THIS ACT, and that is deliberate rather than
  // an omission, exactly as for the four acts before it. Historical constants
  ...
  //
  // THE ASSERTIONS WERE LOCATED BY PREDICATE, NEVER BY LINE NUMBER. v1.0.35
  ...
  // Controller successor act adds commentary above them. v1.0.34 and v1.0.35
  // therefore each record NO line numbers at all — "A convenience that is
  // wrong more often than it is right is not a convenience". Every predicate
  // it enumerates — length against a literal, Set size against a literal, a
  // terminal index pinned to a path, the first out-of-range index pinned to
  // undefined, and every not.toContain / filter-empty assertion naming the
  // absence-control literal — was swept in this file by what it asserts, not
  // by where it sat.
  //
  // NON-ALLOCATION (load-bearing): the v1.0.36 literal below is ONLY the next
  // mechanical absence-control literal.
  //   - v1.0.36 is NOT allocated.
  //   - v1.0.36 is NOT reserved.
  //   - v1.0.36 is NOT an authority.
  //   - v1.0.36 is ONLY the next mechanical absence-control literal.
  // It does NOT allocate, reserve, name-as-allocated, pre-create, grant or
  // otherwise authorize any ODS_V1_MAINTENANCE_ADDENDUM_v1.0.36 artifact.
  // It asserts ABSENCE only. Naming an id in a sequencing/prohibition clause
  // is a PROHIBITION, never an allocation — v1.0.35.json NO_PREALLOCATION
  // states this in its own words, and v1.0.35
  // CONTROLLER_SUCCESSOR_RULE.NEXT_LINEAGE_ALLOCATION_BLOCKS names "THE NEXT
  // ODS LINEAGE ALLOCATION (v1.0.36 and beyond)" as BLOCKED until this very
  // act is integrated — a block is the opposite of an allocation.
  //
  // Measured at THIS candidate's BASE eafaaf57b0ad01031d9c61a985fbc6a2da817340:
  // v1.0.35.json is ALREADY integrated (this is the successor act enumerating
  ...
  // HPO-ODS-W2-32 that v1.0.35 DECLARED has since been REGISTERED by its own
  // separate governed act — measured at this base as 20 PROTECTED_GRANTS rows
  // whose final row is HPO-ODS-W2-32 — so this successor inherits a REGISTERED
  ...
  // tests/ods/ods-scope.test.ts — for exactly FOUR assertions, and that COUNT
  // WAS MEASURED AT THIS HEAD, NOT INHERITED. The predecessor act recorded
  // exactly TWO such assertions. The figure is FOUR here because the v1.0.35
  // ALLOCATION act and the HPO-ODS-W2-32 REGISTRATION act that both landed
  // between the two Controller acts each added their own CONTROLLER-AXIS
  // control (W2-32-ALLOC-N5 and W2-32-REG-N8 respectively) alongside the two
  // inherited ones (CE3-ALLOC-N8 and W2-31-ALLOC-N7). All four read
  // scripts/ods-controller.ts AS TEXT and pin the occurrence count of the
  // absence-control literal to zero, so appending v1.0.35 to the array
  // necessarily turns all four red. Inheriting the predecessor's "exactly two"
  // would have understated the write and left two controls red. All four are
  ...
  // sentinel is touched: PROTECTED_GRANTS is unchanged at 20 rows,
  // HPO-ODS-W2-32 is unaltered, and HPO-ODS-W2-33 remains unallocated and
  ...
  it('does NOT pre-include the next unallocated ODS successor addendum (no automatic inclusion; the literal reserves nothing)', () => {
    expect(IMMUTABLE_BY_CONVENTION).not.toContain('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.36.json')
  })

  // APPEND-OVERSHOOT CONTROL (the successor enumerating v1.0.35): the absence
  // assertion above is a membership test. This one pins the BOUNDARY, so an
  // append beyond the forty-sixth entry cannot pass by simply not being
  // v1.0.36. Length is exact and the final slot is pinned by index, so
  // appending ANY further entry fails here even if the absence literal above
  // were never updated.
  it('APPEND OVERSHOOT: the list ends at exactly 46 entries with v1.0.35 in the final slot', () => {
    expect(IMMUTABLE_BY_CONVENTION.length).toBe(46)
    expect(IMMUTABLE_BY_CONVENTION[45]).toBe('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.35.json')
    expect(IMMUTABLE_BY_CONVENTION[46]).toBeUndefined()
    expect(IMMUTABLE_BY_CONVENTION.filter((e) => e.includes('v1.0.36'))).toEqual([])
  })
  ...
  // NON-ALLOCATION BEHAVIOURAL CONTROL (the successor enumerating v1.0.35,
  // ADVANCED from v1.0.35): the absence-control literal v1.0.36 is NOT
  ...
  it('v1.0.36 is NOT protected — being the absence-control literal confers no protected status', () => {
  ...
  // end-to-end demonstration that v1.0.36 is neither allocated, reserved nor
  ...
  it('a node whose writePaths is exactly v1.0.36 remains SELECTABLE through real decideSelection (not reserved, not protected)', () => {
  ...
  // this control from v1.0.34 to v1.0.35 does not weaken it: v1.0.34 remains
  // pinned by exact position inside the OLD45 literal and its ordered digest
  // below, so a duplicated v1.0.34 still fails the reconstruction controls.
  it('the new entry appears exactly once, and the list carries no duplicates at all', () => {
    const occurrences = IMMUTABLE_BY_CONVENTION.filter(
      (entry) => entry === 'docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.35.json',
    ).length
  ...
  // ---------------------------------------------------------------------
  // ORDER PROOF (ODS_V1_MAINTENANCE_ADDENDUM_v1.0.35.json
  // CONTROLLER_SUCCESSOR_RULE): an independently typed literal of the
  // pre-append 45-entry closed world (OLD45 — never derived from the live
  // IMMUTABLE_BY_CONVENTION import, and never copied from the candidate diff
  // that appends v1.0.35) is hashed with an ordered digest. A remove-only
  // reconstruction of the live, post-append list (dropping exactly the new
  // final element) must reproduce OLD45 element-by-element AND by that same
  ...
  // ---------------------------------------------------------------------
  describe('ORDER PROOF: append-only reconstruction of OLD45', () => {
    // Independently typed — extracted mechanically from the pre-v1.0.35
    // source (the BASE state of scripts/ods-controller.ts at
    // eafaaf57b0ad01031d9c61a985fbc6a2da817340) via
    ...
    //
    // CORROBORATION (independent of this lane): v1.0.35.json states the same
    ...
    // at START_HEAD is 45". The literal below reproduces exactly that: length
    // 45, no duplicates, v1.0.34 last.
    const OLD45: readonly string[] = [
    ...
    // either OLD45 above or the live pre-append 45 entries drifted — never
    ...
    // reproducing BOTH inherited pins below from OLD45.slice(0, -1) and
    // OLD45.slice(0, -2) before this value was pinned. Neither inherited
    // value was copied from its author's work on trust; each was re-derived
    // here from OLD45 and only then compared against the inherited pin.
    const OLD45_DIGEST_EXPECTED = 'd6478a53bece2a6f9e29d97355c6080cbbfb9af4a45063755839a32f9b4fc4ee'

    function orderedDigest(entries: readonly string[]): string {
      return crypto.createHash('sha256').update(JSON.stringify(entries)).digest('hex')
    }

    it('OLD45 literal has length 45, no duplicates, and matches the pinned OLD45_DIGEST', () => {
      expect(OLD45.length).toBe(45)
      expect(new Set(OLD45).size).toBe(45)
      expect(orderedDigest(OLD45)).toBe(OLD45_DIGEST_EXPECTED)
    })

    it('the live list is exactly OLD45 with v1.0.35 appended as the sole new final element', () => {
      expect(IMMUTABLE_BY_CONVENTION.length).toBe(46)
      expect(IMMUTABLE_BY_CONVENTION[45]).toBe('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.35.json')
    })

    it('REMOVE-ONLY RECONSTRUCTION: dropping the final element of the live list reproduces OLD45 element-by-element, in order', () => {
      const reconstructed = IMMUTABLE_BY_CONVENTION.slice(0, -1)
      expect(reconstructed).toEqual(OLD45)
      expect(reconstructed.length).toBe(OLD45.length)
      for (let i = 0; i < OLD45.length; i++) {
        expect(reconstructed[i]).toBe(OLD45[i])
      }
    })

    it('REMOVE-ONLY RECONSTRUCTION: its ordered digest matches OLD45_DIGEST exactly (catches any predecessor reorder)', () => {
      const reconstructed = IMMUTABLE_BY_CONVENTION.slice(0, -1)
      expect(orderedDigest(reconstructed)).toBe(OLD45_DIGEST_EXPECTED)
    })

    // CONTINUITY CONTROL (the successor enumerating v1.0.35): the immediately
    // preceding pin must still be reproducible from OLD45 by remove-only
    // reconstruction. This ties the new OLD45 literal back to the
    // independently-audited predecessor pin, so OLD45 cannot have silently
    // drifted in any of its 44 inherited entries while still satisfying its
    // own freshly-pinned digest. The pin below is the value the predecessor
    // act pinned as its own 44-entry digest constant; it is reproduced here,
    // not inherited on trust.
    //
    // THIS CONSTANT CARRIES NO ORDINAL, AND THE ONE BELOW IT KEEPS ITS OWN.
    // The act that pinned the 44-entry digest took no ordinal — v1.0.34
    ...
    it('CONTINUITY: OLD45 minus its final element reproduces the inherited OLD44 pin exactly', () => {
      const priorClosedWorld = OLD45.slice(0, -1)
      expect(priorClosedWorld.length).toBe(44)
      expect(OLD45[44]).toBe('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.34.json')
      expect(orderedDigest(priorClosedWorld)).toBe(OLD44_DIGEST_INHERITED_PIN)
    })

    // DEEPER CONTINUITY (two generations back): the same remove-only walk
    // reproduces the 43-entry pin. Chaining two inherited pins rather than one
    ...
    it('CONTINUITY: OLD45 minus its final two elements reproduces the inherited OLD43 pin exactly', () => {
      const twoBack = OLD45.slice(0, -2)
      expect(twoBack.length).toBe(43)
      expect(OLD45[43]).toBe('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.33.json')
      expect(orderedDigest(twoBack)).toBe(OLD43_DIGEST_INHERITED_PIN)
    })
    ...
    // construction. Swapping two adjacent OLD45 entries must change the
    ...
    it('MUTATION CONTROL: reordering two predecessor entries changes the ordered digest (order-blind checks would miss this)', () => {
      const reordered = [...OLD45]
      const tmp = reordered[0]
      reordered[0] = reordered[1]
      reordered[1] = tmp
      expect(reordered.length).toBe(OLD45.length)
      expect(new Set(reordered)).toEqual(new Set(OLD45))
      expect(orderedDigest(reordered)).not.toBe(OLD45_DIGEST_EXPECTED)
    })

    // ORDER-CONTINUITY CONTROL (M2 class, in-suite): the controls above are
    // stated over OLD45. This one is stated over a SIMULATED LIVE list — the
    ...
    // between a reorder and a green suite. Every index below is <= 44, i.e.
    // strictly inside the INHERITED range, so the appended final element at
    // index 45 is never the thing being moved.
    ...
    it.each([
      ...
      ['maximally distant inherited entries', 0, 44],
      ['two mid-list inherited addenda', 20, 43],
      ...
      expect(tampered[45]).toBe('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.35.json')

      // The order-sensitive reconstruction is not.
      const reconstructed = tampered.slice(0, -1)
      expect(reconstructed).not.toEqual(OLD45)
      expect(orderedDigest(reconstructed)).not.toBe(OLD45_DIGEST_EXPECTED)
    })
    ...
    it('ORDER CONTINUITY non-vacuity: the same pipeline with no swap reconstructs OLD45 and its digest', () => {
