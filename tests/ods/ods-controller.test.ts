...
describe('E2/CTRL-M3: immutableByConvention closed-world guard', () => {
  it('is the exact 50-member closed world (20 pinned entries + v1.0.10 + v1.0.11 + v1.0.12 + v1.0.13 + v1.0.14 + v1.0.15 + v1.0.16 + v1.0.17 + v1.0.18 + v1.0.19 + v1.0.20 + v1.0.21 + v1.0.22 + v1.0.23 + v1.0.24 + v1.0.25 + v1.0.26 + v1.0.27 + v1.0.28 + v1.0.29 + v1.0.30 + v1.0.31 + v1.0.32 + v1.0.33 + v1.0.34 + v1.0.35 + v1.0.36 + v1.0.37 + v1.0.38 + v1.0.39)', () => {
    expect(IMMUTABLE_BY_CONVENTION.length).toBe(50)
    expect(new Set(IMMUTABLE_BY_CONVENTION).size).toBe(50)
    ...
    expect(IMMUTABLE_BY_CONVENTION).toContain('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.35.json')
    expect(IMMUTABLE_BY_CONVENTION).toContain('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.36.json')
    expect(IMMUTABLE_BY_CONVENTION).toContain('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.37.json')
    expect(IMMUTABLE_BY_CONVENTION).toContain('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.38.json')
    expect(IMMUTABLE_BY_CONVENTION).toContain('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.39.json')
  })
  ...
  // CONTROLLER_42_RULE, v1.0.32.json, v1.0.33.json, v1.0.34.json, v1.0.35.json,
  // v1.0.36.json and v1.0.37.json CONTROLLER_SUCCESSOR_RULE, and now
  // v1.0.38.json CONTROLLER_SUCCESSOR_RULE): the closed world does not
  // pre-include a later ODS successor artifact. The control ADVANCES with the
  // list — v1.0.38 is now enumerated by this successor act, so the absence
  // assertion moves to the mechanically-next version. This is exactly the act
  // v1.0.38.json CONTROLLER_SUCCESSOR_RULE.requirement names in its own words:
  // "The NEXT Controller successor act is REQUIRED to enumerate docs/ops/ods/
  // ODS_V1_MAINTENANCE_ADDENDUM_v1.0.38.json in IMMUTABLE_BY_CONVENTION,
  // taking the THEN-LIVE array length to length + EXACTLY ONE, making v1.0.38
  // FINAL, and preserving the existing order of every predecessor entry." It
  // must also advance every length, Set-size and index predicate that the
  // Controller suite binds to the array.
  //
  // THE THEN-LIVE LENGTH WAS RE-DERIVED AT THIS ACT'S OWN HEAD, not inherited.
  // v1.0.38 CONTROLLER_SUCCESSOR_RULE.NO_CONTROLLER_ORDINAL_RESERVED is binding
  // in its own words: "nothing is reserved. This artifact does not name,
  // number, label or otherwise reserve the future Controller successor act.
  // The act is identified only by the array state it must produce." Measured
  // here at BASE 9ba5fac7b5e1b3938f0c68d10fa688c045df5cec by structural
  // bracket matching of scripts/ods-controller.ts — 48 entries, 48 unique,
  // final entry v1.0.37 — which is what makes 49 below a derived figure
  // rather than a remembered one.
  //
  // NO ORDINAL LABEL IS USED FOR THIS ACT, and that is deliberate rather than
  // an omission, exactly as for the seven acts before it. Historical constants
  ...
  //
  // THE ASSERTIONS WERE LOCATED BY PREDICATE, NEVER BY LINE NUMBER. v1.0.38
  // CONTROLLER_SUCCESSOR_RULE.SWEEP_BY_PREDICATE_NOT_BY_LINE_NUMBER makes this
  // binding and gives the reason: line anchors recorded by earlier acts had
  // each already gone stale by the time the next act ran, because a Controller
  // successor act adds commentary above them. This act therefore records NO
  // line numbers at all — "A convenience that is wrong more often than it is
  // right is not a convenience". Every predicate it enumerates — length
  // against a literal, Set size against a literal, a terminal index pinned to
  // a path, the first out-of-range index pinned to undefined, and every
  // not.toContain / filter-empty assertion naming the absence-control literal
  // — was swept in this file by what it asserts, not by where it sat.
  //
  // NON-ALLOCATION (load-bearing): the v1.0.39 literal below is ONLY the next
  // mechanical absence-control literal.
  //   - v1.0.39 is NOT allocated.
  //   - v1.0.39 is NOT reserved.
  //   - v1.0.39 is NOT an authority.
  //   - v1.0.39 is ONLY the next mechanical absence-control literal.
  // It does NOT allocate, reserve, name-as-allocated, pre-create, grant or
  // otherwise authorize any ODS_V1_MAINTENANCE_ADDENDUM_v1.0.39 artifact.
  // It asserts ABSENCE only. Naming an id in a sequencing/prohibition clause
  // is a PROHIBITION, never an allocation — v1.0.38.json NO_PREALLOCATION
  // (via IT_MUST_NOT_ALLOCATE_A_LATER_ODS_VERSION) states this in its own
  // words, and v1.0.38 CONTROLLER_SUCCESSOR_RULE.NEXT_LINEAGE_ALLOCATION_BLOCKS
  // names "THE NEXT ODS LINEAGE ALLOCATION (v1.0.39 and beyond)" as BLOCKED
  // until this very act is integrated — a block is the opposite of an
  // allocation.
  //
  // Measured at THIS candidate's BASE 9ba5fac7b5e1b3938f0c68d10fa688c045df5cec:
  // v1.0.38.json is ALREADY on the filesystem but NOT YET integrated into
  // IMMUTABLE_BY_CONVENTION (this is the successor act performing exactly that
  // enumeration, per CONTROLLER_SUCCESSOR_RULE.NOT_ALLOCATED_HERE, which
  // confirms the v1.0.38 allocation lane did NOT edit scripts/ods-controller.ts
  // or this file). The grant HPO-ODS-W2-35 that v1.0.38 DECLARED has since been
  // REGISTERED by its own separate governed act — measured at this base as 23
  // PROTECTED_GRANTS rows whose final row is HPO-ODS-W2-35 — so this successor
  // inherits a REGISTERED grant, not a pending one.
  //
  // THIS ACT DOES NOT TOUCH scripts/ods-scope.ts, but it DOES touch
  // tests/ods/ods-scope.test.ts — for exactly NINE assertions, and that COUNT
  // WAS MEASURED AT THIS HEAD, NOT INHERITED. The predecessor act recorded
  // exactly EIGHT such assertions (CE3-ALLOC-N8, W2-31-ALLOC-N7,
  // W2-32-ALLOC-N5, W2-32-REG-N8, W2-33-ALLOC-N4, W2-33-REG-N8,
  // W2-34-ALLOC-N4, W2-34-REG-N8). The figure is NINE here because the
  // HPO-ODS-W2-35 REGISTRATION act (W2-35-REG-N8), which landed between the
  // two Controller acts, added its own CONTROLLER-AXIS control alongside the
  // eight inherited ones. All nine read scripts/ods-controller.ts AS TEXT and
  // pin the occurrence count of the absence-control literal to zero, so
  // appending v1.0.38 to the array necessarily turns all nine red. Inheriting
  // the predecessor's "exactly eight" would have understated the write and
  // left one control red. All nine are assertions about the ARRAY, so only a
  // Controller act can move them, and this is that act. No registration act
  // is performed and no registry-axis sentinel is touched: PROTECTED_GRANTS
  // is unchanged at 23 rows, HPO-ODS-W2-35 is unaltered, and HPO-ODS-W2-36
  // remains unallocated and unregistered, with zero occurrences in the scope
  // registry.
  //
  ...
  it('does NOT pre-include the next unallocated ODS successor addendum (no automatic inclusion; the literal reserves nothing)', () => {
    expect(IMMUTABLE_BY_CONVENTION).not.toContain('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.40.json')
  })

  // APPEND-OVERSHOOT CONTROL (the successor enumerating v1.0.38): the absence
  // assertion above is a membership test. This one pins the BOUNDARY, so an
  // append beyond the forty-ninth entry cannot pass by simply not being
  // v1.0.39. Length is exact and the final slot is pinned by index, so
  // appending ANY further entry fails here even if the absence literal above
  // were never updated.
  it('APPEND OVERSHOOT: the list ends at exactly 50 entries with v1.0.39 in the final slot', () => {
    expect(IMMUTABLE_BY_CONVENTION.length).toBe(50)
    expect(IMMUTABLE_BY_CONVENTION[49]).toBe('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.39.json')
    expect(IMMUTABLE_BY_CONVENTION[50]).toBeUndefined()
    expect(IMMUTABLE_BY_CONVENTION.filter((e) => e.includes('v1.0.40'))).toEqual([])
  })
  ...
  // ODS_V1_MAINTENANCE_ADDENDUM_v1.0.32.json CONTROLLER_SUCCESSOR_RULE
  // .requirement ("The NEXT Controller successor act is REQUIRED to enumerate
  // docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.32.json in
  // IMMUTABLE_BY_CONVENTION after this successor is integrated, taking the
  // array from 42 entries to 43 with v1.0.32 as the final item"): the
  // Controller must enumerate its own governing addendum, and that enumeration
  // is a SEPARATE governed maintenance mission from the addendum that
  // allocated the lineage. v1.0.32 is explicitly NOT self-including, and its
  // CONTROLLER_SUCCESSOR_RULE.NOT_ALLOCATED_HERE clause forbade that lane from
  // touching scripts/ods-controller.ts or this file at all. These controls
  // prove the newly appended entry is actually WIRED INTO decideSelection
  // through the REAL guard, not merely present as a string — removing it from
  // IMMUTABLE_BY_CONVENTION fails them independently of the length/Set/digest
  // assertions elsewhere in this file.
  //
  // THIS BLOCK INVERTS TWO CONTROLS that stood here before this act. While
  // v1.0.32 was the absence-control literal, the suite asserted it was NOT
  ...
  // ADVANCE to v1.0.33 (below) and these four take their place for v1.0.32.
  // The inversion is the substance of the enumeration, not a regression in it:
  // v1.0.32 CONTROLLER_SUCCESSOR_RULE.MATERIALIZING_THIS_FILE_MOVES_NO_EXISTING
  // _ASSERTION named exactly these two controls (:783 and :796 as that artifact
  // measured them) as the ones its own materialization did not move — because
  // they are assertions about the ARRAY, and only a Controller act that changes
  // the ARRAY can move them. This is that act.
  it('a node targeting v1.0.32 STOPs with PROTECTED_SURFACE_CHANGE via real decideSelection', () => {
  ...
  it('a case-mutated spelling of the new entry (v1.0.32) is NONCANONICAL_PROTECTED_PATH, never PROTECTED_SURFACE_CHANGE', () => {
    const unit = baseUnit({ writePaths: ['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_V1.0.32.JSON'] })
    const decision = decideSelection(unit, 'OPEN', {}, {})
    expect(decision.selectable).toBe(false)
    expect(decision.stopClass).toBe('NONCANONICAL_PROTECTED_PATH')
  ...
  // decideSelection. A guard that silently lowercased would report
  // violated=true here and fail.
  it('checkImmutableGuard classifies the case-mutated v1.0.32 spelling as noncanonical, never as a violation', () => {
    const guard = checkImmutableGuard(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_V1.0.32.JSON'])
    expect(guard.violated).toBe(false)
    expect(guard.violatingPaths).toEqual([])
    expect(guard.nonCanonicalPaths).toEqual(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_V1.0.32.JSON'])
  })
  ...
  // above this proves the guard distinguishes canonical from case-mutated
  // rather than answering the same way to both.
  it('checkImmutableGuard treats the canonical v1.0.32 spelling as a real protected-surface violation', () => {
    const guard = checkImmutableGuard(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.32.json'])
    expect(guard.violated).toBe(true)
    expect(guard.violatingPaths).toEqual(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.32.json'])
    expect(guard.nonCanonicalPaths).toEqual([])
  })

  // ODS_V1_MAINTENANCE_ADDENDUM_v1.0.33.json CONTROLLER_SUCCESSOR_RULE
  // .requirement ("The NEXT Controller successor act is REQUIRED to enumerate
  // docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.33.json in
  // IMMUTABLE_BY_CONVENTION after this artifact is integrated, taking the array
  // from its then-live length to that length PLUS EXACTLY ONE, with v1.0.33 as
  // the FINAL item"): the Controller must enumerate its own governing addendum,
  // and that enumeration is a SEPARATE governed maintenance mission from the
  // addendum that allocated the lineage. v1.0.33 is explicitly NOT
  ...
  //
  // THIS BLOCK INVERTS TWO CONTROLS that stood here before this act. While
  // v1.0.33 was the absence-control literal, the suite asserted it was NOT
  // protected and that a node writing it remained SELECTABLE. Enumerating it
  // necessarily makes it protected and unselectable, so those two controls
  // ADVANCE to v1.0.34 (below) and these four take their place for v1.0.33.
  // The inversion is the substance of the enumeration, not a regression in it:
  // v1.0.33 CONTROLLER_SUCCESSOR_RULE
  // .MATERIALIZING_THIS_FILE_MOVES_EXACTLY_ONE_EXISTING_ASSERTION_AND_IT_IS_NOT
  // _A_CONTROLLER_ONE named exactly this class of control in its own words —
  // "Every Controller assertion naming v1.0.33 is an assertion about the ARRAY
  // ... NONE of them is a filesystem-existence assertion, so creating this FILE
  // moves none of them" — as the ones its own materialization did not move.
  // Only a Controller act that changes the ARRAY can move them, and this is
  // that act.
  it('a node targeting v1.0.33 STOPs with PROTECTED_SURFACE_CHANGE via real decideSelection', () => {
    const unit = baseUnit({ writePaths: ['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.33.json'] })
    const decision = decideSelection(unit, 'OPEN', {}, {})
    expect(decision.selectable).toBe(false)
    expect(decision.stopClass).toBe('PROTECTED_SURFACE_CHANGE')
  })

  it('a case-mutated spelling of the new entry (v1.0.33) is NONCANONICAL_PROTECTED_PATH, never PROTECTED_SURFACE_CHANGE', () => {
    const unit = baseUnit({ writePaths: ['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_V1.0.33.JSON'] })
    const decision = decideSelection(unit, 'OPEN', {}, {})
    expect(decision.selectable).toBe(false)
    expect(decision.stopClass).toBe('NONCANONICAL_PROTECTED_PATH')
  ...
  // decideSelection. A guard that silently lowercased would report
  // violated=true here and fail.
  it('checkImmutableGuard classifies the case-mutated v1.0.33 spelling as noncanonical, never as a violation', () => {
    const guard = checkImmutableGuard(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_V1.0.33.JSON'])
    expect(guard.violated).toBe(false)
    expect(guard.violatingPaths).toEqual([])
    expect(guard.nonCanonicalPaths).toEqual(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_V1.0.33.JSON'])
  })
  ...
  // above this proves the guard distinguishes canonical from case-mutated
  // rather than answering the same way to both.
  it('checkImmutableGuard treats the canonical v1.0.33 spelling as a real protected-surface violation', () => {
    const guard = checkImmutableGuard(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.33.json'])
    expect(guard.violated).toBe(true)
    expect(guard.violatingPaths).toEqual(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.33.json'])
    expect(guard.nonCanonicalPaths).toEqual([])
  })
  ...
  // ODS_V1_MAINTENANCE_ADDENDUM_v1.0.35.json CONTROLLER_SUCCESSOR_RULE
  // .requirement ("The NEXT Controller successor act is REQUIRED to enumerate
  // docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.35.json in
  // IMMUTABLE_BY_CONVENTION after this artifact is integrated, taking the array
  // from its then-live length to that length PLUS EXACTLY ONE, with v1.0.35 as
  // the FINAL item"): the Controller must enumerate its own governing addendum,
  // and that enumeration is a SEPARATE governed maintenance mission from the
  // addendum that allocated the lineage. v1.0.35 is explicitly NOT
  // self-including, and its CONTROLLER_SUCCESSOR_RULE.NOT_ALLOCATED_HERE clause
  // forbade that lane from touching scripts/ods-controller.ts or this file at
  // all. These controls prove the newly appended entry is actually WIRED INTO
  // decideSelection through the REAL guard, not merely present as a string —
  ...
  //
  // THIS BLOCK INVERTS TWO CONTROLS that stood here before this act. While
  // v1.0.35 was the absence-control literal, the suite asserted it was NOT
  // protected and that a node writing it remained SELECTABLE. Enumerating it
  // necessarily makes it protected and unselectable, so those two controls
  // ADVANCE to v1.0.36 (below) and these four take their place for v1.0.35.
  // The inversion is the substance of the enumeration, not a regression in it:
  // v1.0.35 CONTROLLER_SUCCESSOR_RULE
  // .MATERIALIZING_THIS_FILE_MOVES_EXISTING_ASSERTIONS_AND_THEY_ARE_NOT_CONTROLLER
  // _ONES named exactly this class of control in its own words — "Every
  // Controller assertion naming v1.0.35 is an assertion about the ARRAY ...
  // NONE of them is a filesystem-existence assertion, so creating this FILE
  // moves none of them" — as the ones its own materialization did not move.
  // Only a Controller act that changes the ARRAY can move them, and this is
  // that act.
  it('a node targeting v1.0.35 STOPs with PROTECTED_SURFACE_CHANGE via real decideSelection', () => {
  ...
  it('a case-mutated spelling of the new entry (v1.0.35) is NONCANONICAL_PROTECTED_PATH, never PROTECTED_SURFACE_CHANGE', () => {
    const unit = baseUnit({ writePaths: ['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_V1.0.35.JSON'] })
    const decision = decideSelection(unit, 'OPEN', {}, {})
    expect(decision.selectable).toBe(false)
    expect(decision.stopClass).toBe('NONCANONICAL_PROTECTED_PATH')
  ...
  // decideSelection. A guard that silently lowercased would report
  // violated=true here and fail.
  it('checkImmutableGuard classifies the case-mutated v1.0.35 spelling as noncanonical, never as a violation', () => {
    const guard = checkImmutableGuard(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_V1.0.35.JSON'])
    expect(guard.violated).toBe(false)
    expect(guard.violatingPaths).toEqual([])
    expect(guard.nonCanonicalPaths).toEqual(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_V1.0.35.JSON'])
  })
  ...
  // above this proves the guard distinguishes canonical from case-mutated
  // rather than answering the same way to both.
  it('checkImmutableGuard treats the canonical v1.0.35 spelling as a real protected-surface violation', () => {
    const guard = checkImmutableGuard(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.35.json'])
    expect(guard.violated).toBe(true)
    expect(guard.violatingPaths).toEqual(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.35.json'])
    expect(guard.nonCanonicalPaths).toEqual([])
  })

  // ODS_V1_MAINTENANCE_ADDENDUM_v1.0.38.json CONTROLLER_SUCCESSOR_RULE
  // .requirement ("The NEXT Controller successor act is REQUIRED to enumerate
  // docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.38.json in
  // IMMUTABLE_BY_CONVENTION, taking the THEN-LIVE array length to length PLUS
  // EXACTLY ONE, making v1.0.38 FINAL, and preserving the existing order of
  // every predecessor entry"): the Controller must enumerate its own governing
  // addendum, and that enumeration is a SEPARATE governed maintenance mission
  // from the addendum that allocated the lineage. v1.0.38 is explicitly NOT
  // self-including, and its CONTROLLER_SUCCESSOR_RULE.NOT_ALLOCATED_HERE clause
  // forbade that lane from touching scripts/ods-controller.ts or this file at
  // all. These controls prove the newly appended entry is actually WIRED INTO
  // decideSelection through the REAL guard, not merely present as a string —
  ...
  //
  // THIS BLOCK INVERTS TWO CONTROLS that stood here before this act. While
  // v1.0.38 was the absence-control literal, the suite asserted it was NOT
  // protected and that a node writing it remained SELECTABLE. Enumerating it
  // necessarily makes it protected and unselectable, so those two controls
  // ADVANCE to v1.0.39 (below) and these four take their place for v1.0.38.
  // The inversion is the substance of the enumeration, not a regression in it:
  // v1.0.38 CONTROLLER_SUCCESSOR_RULE
  // .FILESYSTEM_EXISTENCE_AND_CONTROLLER_PROTECTED_STATUS_ARE_DIFFERENT_AXES
  // named exactly this class of control in its own words — "Creating the file
  // changes the FILESYSTEM axis. It does not change the CONTROLLER axis." —
  // and it is precisely this Controller act, changing the ARRAY, that moves
  // v1.0.38 onto the CONTROLLER axis for the first time.
  it('a node targeting v1.0.38 STOPs with PROTECTED_SURFACE_CHANGE via real decideSelection', () => {
    const unit = baseUnit({ writePaths: ['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.38.json'] })
    const decision = decideSelection(unit, 'OPEN', {}, {})
    expect(decision.selectable).toBe(false)
    expect(decision.stopClass).toBe('PROTECTED_SURFACE_CHANGE')
  })

  it('a case-mutated spelling of the new entry (v1.0.38) is NONCANONICAL_PROTECTED_PATH, never PROTECTED_SURFACE_CHANGE', () => {
    const unit = baseUnit({ writePaths: ['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_V1.0.38.JSON'] })
    const decision = decideSelection(unit, 'OPEN', {}, {})
    expect(decision.selectable).toBe(false)
    expect(decision.stopClass).toBe('NONCANONICAL_PROTECTED_PATH')
  ...
  // decideSelection. A guard that silently lowercased would report
  // violated=true here and fail.
  it('checkImmutableGuard classifies the case-mutated v1.0.38 spelling as noncanonical, never as a violation', () => {
    const guard = checkImmutableGuard(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_V1.0.38.JSON'])
    expect(guard.violated).toBe(false)
    expect(guard.violatingPaths).toEqual([])
    expect(guard.nonCanonicalPaths).toEqual(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_V1.0.38.JSON'])
  })
  ...
  // above this proves the guard distinguishes canonical from case-mutated
  // rather than answering the same way to both.
  it('checkImmutableGuard treats the canonical v1.0.38 spelling as a real protected-surface violation', () => {
    const guard = checkImmutableGuard(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.38.json'])
    expect(guard.violated).toBe(true)
    expect(guard.violatingPaths).toEqual(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.38.json'])
    expect(guard.nonCanonicalPaths).toEqual([])
  })

  // ODS_V1_MAINTENANCE_ADDENDUM_v1.0.39.json CONTROLLER_SUCCESSOR_RULE
  // .requirement ("The NEXT Controller successor act is REQUIRED to enumerate
  // docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.39.json in
  // IMMUTABLE_BY_CONVENTION, taking the THEN-LIVE array length to length PLUS
  // EXACTLY ONE, making v1.0.39 FINAL, and preserving the existing order of
  // every predecessor entry"): the Controller must enumerate its own governing
  // addendum, and that enumeration is a SEPARATE governed maintenance mission
  // from the addendum that allocated the lineage (CV1-SECURITY-ACL-ODS-
  // ALLOCATION-R1, which declared HPO-ODS-W2-36 but explicitly did NOT touch
  // scripts/ods-controller.ts or this file). These controls prove the newly
  // appended entry is actually WIRED INTO decideSelection through the REAL
  // guard, not merely present as a string — removing it from
  // IMMUTABLE_BY_CONVENTION fails them independently of the length/Set/digest
  // assertions elsewhere in this file.
  //
  // THIS BLOCK INVERTS TWO CONTROLS that stood here before this act. While
  // v1.0.39 was the absence-control literal, the suite asserted it was NOT
  // protected and that a node writing it remained SELECTABLE. Enumerating it
  // necessarily makes it protected and unselectable, so those two controls
  // ADVANCE to v1.0.40 (below) and these four take their place for v1.0.39.
  it('a node targeting v1.0.39 STOPs with PROTECTED_SURFACE_CHANGE via real decideSelection', () => {
    const unit = baseUnit({ writePaths: ['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.39.json'] })
    const decision = decideSelection(unit, 'OPEN', {}, {})
    expect(decision.selectable).toBe(false)
    expect(decision.stopClass).toBe('PROTECTED_SURFACE_CHANGE')
  })

  it('a case-mutated spelling of the new entry (v1.0.39) is NONCANONICAL_PROTECTED_PATH, never PROTECTED_SURFACE_CHANGE', () => {
    const unit = baseUnit({ writePaths: ['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_V1.0.39.JSON'] })
    const decision = decideSelection(unit, 'OPEN', {}, {})
    expect(decision.selectable).toBe(false)
    expect(decision.stopClass).toBe('NONCANONICAL_PROTECTED_PATH')
  ...
  // decideSelection. A guard that silently lowercased would report
  // violated=true here and fail.
  it('checkImmutableGuard classifies the case-mutated v1.0.39 spelling as noncanonical, never as a violation', () => {
    const guard = checkImmutableGuard(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_V1.0.39.JSON'])
    expect(guard.violated).toBe(false)
    expect(guard.violatingPaths).toEqual([])
    expect(guard.nonCanonicalPaths).toEqual(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_V1.0.39.JSON'])
  })
  ...
  // above this proves the guard distinguishes canonical from case-mutated
  // rather than answering the same way to both.
  it('checkImmutableGuard treats the canonical v1.0.39 spelling as a real protected-surface violation', () => {
    const guard = checkImmutableGuard(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.39.json'])
    expect(guard.violated).toBe(true)
    expect(guard.violatingPaths).toEqual(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.39.json'])
    expect(guard.nonCanonicalPaths).toEqual([])
  })

  // NON-ALLOCATION BEHAVIOURAL CONTROL (the successor enumerating v1.0.39,
  // ADVANCED from v1.0.39): the absence-control literal v1.0.40 is NOT
  ...
  it('v1.0.40 is NOT protected — being the absence-control literal confers no protected status', () => {
    const guard = checkImmutableGuard(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.40.json'])
    expect(guard.violated).toBe(false)
    expect(guard.violatingPaths).toEqual([])
    expect(guard.nonCanonicalPaths).toEqual([])
  })
  ...
  // end-to-end demonstration that v1.0.40 is neither allocated, reserved nor
  ...
  it('a node whose writePaths is exactly v1.0.40 remains SELECTABLE through real decideSelection (not reserved, not protected)', () => {
    const unit = baseUnit({ writePaths: ['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.40.json'] })
    const decision = decideSelection(unit, 'OPEN', {}, {})
    expect(decision.selectable).toBe(true)
  ...
  // this control from v1.0.38 to v1.0.39 does not weaken it: v1.0.38 remains
  // pinned by exact position inside the OLD49 literal and its ordered digest
  // below, so a duplicated v1.0.38 still fails the reconstruction controls.
  it('the new entry appears exactly once, and the list carries no duplicates at all', () => {
    const occurrences = IMMUTABLE_BY_CONVENTION.filter(
      (entry) => entry === 'docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.39.json',
    ).length
  ...
  // ---------------------------------------------------------------------
  // ORDER PROOF (ODS_V1_MAINTENANCE_ADDENDUM_v1.0.39.json
  // CONTROLLER_SUCCESSOR_RULE): an independently typed literal of the
  // pre-append 49-entry closed world (OLD49 — never derived from the live
  // IMMUTABLE_BY_CONVENTION import, and never copied from the candidate diff
  // that appends v1.0.39) is hashed with an ordered digest. A remove-only
  // reconstruction of the live, post-append list (dropping exactly the new
  // final element) must reproduce OLD49 element-by-element AND by that same
  ...
  // ---------------------------------------------------------------------
  describe('ORDER PROOF: append-only reconstruction of OLD49', () => {
    // Independently typed — extracted mechanically from the pre-v1.0.39
    // source (the BASE state of scripts/ods-controller.ts at
    // 92dbadbd44a9916561a26ba7084d21ff8ac7b63e) via
    ...
    // the live import.
    //
    // THE EXTRACTION AVOIDED A KNOWN PARSE TRAP. The declaration reads
    // `IMMUTABLE_BY_CONVENTION: readonly string[] = [`, so a parser that takes
    // the first `[` after the identifier lands inside the TYPE ANNOTATION and
    ...
    //
    // CORROBORATION (independent of this lane): v1.0.39.json's own
    // CONTROLLER_SUCCESSOR_RULE records that the future act "measures the
    // live length at its own head". The literal below reproduces exactly
    // that: length 49, no duplicates, v1.0.38 last.
    const OLD49: readonly string[] = [
    ...
    // either OLD49 above or the live pre-append 49 entries drifted — never
    ...
    // reproducing BOTH inherited pins below from OLD49.slice(0, -1) and
    // OLD49.slice(0, -2) before this value was pinned. Neither inherited
    // value was copied from its author's work on trust; each was re-derived
    // here from OLD49 and only then compared against the inherited pin.
    const OLD49_DIGEST_EXPECTED = 'd1cdb7b333af8da181d460e817b3c5ffb9bbee387111a8350fb22ed5a8ec1951'

    function orderedDigest(entries: readonly string[]): string {
      return crypto.createHash('sha256').update(JSON.stringify(entries)).digest('hex')
    }

    it('OLD49 literal has length 49, no duplicates, and matches the pinned OLD49_DIGEST', () => {
      expect(OLD49.length).toBe(49)
      expect(new Set(OLD49).size).toBe(49)
      expect(orderedDigest(OLD49)).toBe(OLD49_DIGEST_EXPECTED)
    })

    it('the live list is exactly OLD49 with v1.0.39 appended as the sole new final element', () => {
      expect(IMMUTABLE_BY_CONVENTION.length).toBe(50)
      expect(IMMUTABLE_BY_CONVENTION[49]).toBe('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.39.json')
    })

    it('REMOVE-ONLY RECONSTRUCTION: dropping the final element of the live list reproduces OLD49 element-by-element, in order', () => {
      const reconstructed = IMMUTABLE_BY_CONVENTION.slice(0, -1)
      expect(reconstructed).toEqual(OLD49)
      expect(reconstructed.length).toBe(OLD49.length)
      for (let i = 0; i < OLD49.length; i++) {
        expect(reconstructed[i]).toBe(OLD49[i])
      }
    })

    it('REMOVE-ONLY RECONSTRUCTION: its ordered digest matches OLD49_DIGEST exactly (catches any predecessor reorder)', () => {
      const reconstructed = IMMUTABLE_BY_CONVENTION.slice(0, -1)
      expect(orderedDigest(reconstructed)).toBe(OLD49_DIGEST_EXPECTED)
    })

    // CONTINUITY CONTROL (the successor enumerating v1.0.39): the immediately
    // preceding pin must still be reproducible from OLD49 by remove-only
    // reconstruction. This ties the new OLD49 literal back to the
    // independently-audited predecessor pin, so OLD49 cannot have silently
    // drifted in any of its 48 inherited entries while still satisfying its
    // own freshly-pinned digest. The pin below is the value the predecessor
    // act pinned as its own 48-entry digest constant; it is reproduced here,
    // not inherited on trust.
    //
    // THIS CONSTANT CARRIES NO ORDINAL, AND THE ONE BELOW IT KEEPS ITS OWN.
    // The act that pinned the 48-entry digest took no ordinal — its own
    ...
    const OLD48_DIGEST_INHERITED_PIN = 'b9a06908e84496ab201e30d1dad52d8bc74def42efc683ce64c1ae2ee11f6b14'

    it('CONTINUITY: OLD49 minus its final element reproduces the inherited OLD48 pin exactly', () => {
      const priorClosedWorld = OLD49.slice(0, -1)
      expect(priorClosedWorld.length).toBe(48)
      expect(OLD49[48]).toBe('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.38.json')
      expect(orderedDigest(priorClosedWorld)).toBe(OLD48_DIGEST_INHERITED_PIN)
    })

    // DEEPER CONTINUITY (two generations back): the same remove-only walk
    // reproduces the 47-entry pin. Chaining two inherited pins rather than one
    // means a drifted inherited entry would have to satisfy BOTH
    // independently-audited historical digests to pass unnoticed.
    const OLD47_DIGEST_INHERITED_PIN = '7186da080af7cfa00bc6a2be05eba66810552f13b6f94b62685d632793b40ae4'

    it('CONTINUITY: OLD49 minus its final two elements reproduces the inherited OLD47 pin exactly', () => {
      const twoBack = OLD49.slice(0, -2)
      expect(twoBack.length).toBe(47)
      expect(OLD49[47]).toBe('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.37.json')
      expect(orderedDigest(twoBack)).toBe(OLD47_DIGEST_INHERITED_PIN)
    })
    ...
    // construction. Swapping two adjacent OLD49 entries must change the
    ...
    it('MUTATION CONTROL: reordering two predecessor entries changes the ordered digest (order-blind checks would miss this)', () => {
      const reordered = [...OLD49]
      const tmp = reordered[0]
      reordered[0] = reordered[1]
      reordered[1] = tmp
      expect(reordered.length).toBe(OLD49.length)
      expect(new Set(reordered)).toEqual(new Set(OLD49))
      expect(orderedDigest(reordered)).not.toBe(OLD49_DIGEST_EXPECTED)
    })

    // ORDER-CONTINUITY CONTROL (M2 class, in-suite): the controls above are
    // stated over OLD49. This one is stated over a SIMULATED LIVE list — the
    ...
    // between a reorder and a green suite. Every index below is <= 48, i.e.
    // strictly inside the INHERITED range, so the appended final element at
    // index 49 is never the thing being moved.
    ...
    it.each([
      ...
      ['maximally distant inherited entries', 0, 48],
      ['two mid-list inherited addenda', 20, 47],
      ...
      expect(tampered[49]).toBe('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.39.json')

      // The order-sensitive reconstruction is not.
      const reconstructed = tampered.slice(0, -1)
      expect(reconstructed).not.toEqual(OLD49)
      expect(orderedDigest(reconstructed)).not.toBe(OLD49_DIGEST_EXPECTED)
    })
    ...
    it('ORDER CONTINUITY non-vacuity: the same pipeline with no swap reconstructs OLD49 and its digest', () => {
