...
describe('E2/CTRL-M3: immutableByConvention closed-world guard', () => {
  it('is the exact 47-member closed world (20 pinned entries + v1.0.10 + v1.0.11 + v1.0.12 + v1.0.13 + v1.0.14 + v1.0.15 + v1.0.16 + v1.0.17 + v1.0.18 + v1.0.19 + v1.0.20 + v1.0.21 + v1.0.22 + v1.0.23 + v1.0.24 + v1.0.25 + v1.0.26 + v1.0.27 + v1.0.28 + v1.0.29 + v1.0.30 + v1.0.31 + v1.0.32 + v1.0.33 + v1.0.34 + v1.0.35 + v1.0.36)', () => {
    expect(IMMUTABLE_BY_CONVENTION.length).toBe(47)
    expect(new Set(IMMUTABLE_BY_CONVENTION).size).toBe(47)
  ...
  // CONTROLLER_42_RULE, v1.0.32.json, v1.0.33.json, v1.0.34.json and
  // v1.0.35.json CONTROLLER_SUCCESSOR_RULE, and now v1.0.36.json
  // CONTROLLER_SUCCESSOR_RULE): the closed world does not pre-include a later
  // ODS successor artifact. The control ADVANCES with the list — v1.0.36 is
  // now enumerated by this successor act, so the absence assertion moves to
  // the mechanically-next version. This is exactly the act v1.0.36.json
  ...
  // ODS_V1_MAINTENANCE_ADDENDUM_v1.0.36.json in IMMUTABLE_BY_CONVENTION,
  // taking the THEN-LIVE array length to length + EXACTLY ONE, making v1.0.36
  ...
  // v1.0.36 CONTROLLER_SUCCESSOR_RULE.NO_CONTROLLER_ORDINAL_RESERVED is binding
  ...
  // here at BASE e7df0c9a558de0481f966008f0908bb3ee084811 by structural
  // bracket matching of scripts/ods-controller.ts — 46 entries, 46 unique,
  // final entry v1.0.35 — which is what makes 47 below a derived figure
  // rather than a remembered one.
  //
  // NO ORDINAL LABEL IS USED FOR THIS ACT, and that is deliberate rather than
  // an omission, exactly as for the five acts before it. Historical constants
  ...
  //
  // THE ASSERTIONS WERE LOCATED BY PREDICATE, NEVER BY LINE NUMBER. v1.0.36
  ...
  //
  // NON-ALLOCATION (load-bearing): the v1.0.37 literal below is ONLY the next
  // mechanical absence-control literal.
  //   - v1.0.37 is NOT allocated.
  //   - v1.0.37 is NOT reserved.
  //   - v1.0.37 is NOT an authority.
  //   - v1.0.37 is ONLY the next mechanical absence-control literal.
  // It does NOT allocate, reserve, name-as-allocated, pre-create, grant or
  // otherwise authorize any ODS_V1_MAINTENANCE_ADDENDUM_v1.0.37 artifact.
  // It asserts ABSENCE only. Naming an id in a sequencing/prohibition clause
  // is a PROHIBITION, never an allocation — v1.0.36.json NO_PREALLOCATION
  // (via IT_MUST_NOT_ALLOCATE_A_LATER_ODS_VERSION) states this in its own
  // words, and v1.0.36 CONTROLLER_SUCCESSOR_RULE.NEXT_LINEAGE_ALLOCATION_BLOCKS
  // names "THE NEXT ODS LINEAGE ALLOCATION (v1.0.37 and beyond)" as BLOCKED
  // until this very act is integrated — a block is the opposite of an
  // allocation.
  //
  // Measured at THIS candidate's BASE e7df0c9a558de0481f966008f0908bb3ee084811:
  // v1.0.36.json is ALREADY on the filesystem but NOT YET integrated into
  ...
  // confirms the v1.0.36 allocation lane did NOT edit scripts/ods-controller.ts
  // or this file). The grant HPO-ODS-W2-33 that v1.0.36 DECLARED has since been
  // REGISTERED by its own separate governed act (PR #182) — measured at this
  // base as 21 PROTECTED_GRANTS rows whose final row is HPO-ODS-W2-33 — so this
  // successor inherits a REGISTERED grant, not a pending one.
  //
  // THIS ACT DOES NOT TOUCH scripts/ods-scope.ts, but it DOES touch
  // tests/ods/ods-scope.test.ts — for exactly SIX assertions, and that COUNT
  // WAS MEASURED AT THIS HEAD, NOT INHERITED. The predecessor act recorded
  // exactly FOUR such assertions. The figure is SIX here because the v1.0.36
  // ALLOCATION act (W2-33-ALLOC-N4) and the HPO-ODS-W2-33 REGISTRATION act
  // (W2-33-REG-N8) that both landed between the two Controller acts each added
  // their own CONTROLLER-AXIS control alongside the four inherited ones
  // (CE3-ALLOC-N8, W2-31-ALLOC-N7, W2-32-ALLOC-N5, W2-32-REG-N8). All six read
  // scripts/ods-controller.ts AS TEXT and pin the occurrence count of the
  // absence-control literal to zero, so appending v1.0.36 to the array
  // necessarily turns all six red. Inheriting the predecessor's "exactly four"
  // would have understated the write and left two controls red. All six are
  // assertions about the ARRAY, so only a Controller act can move them, and
  // this is that act. No registration act is performed and no registry-axis
  // sentinel is touched: PROTECTED_GRANTS is unchanged at 21 rows,
  // HPO-ODS-W2-33 is unaltered, and HPO-ODS-W2-34 remains unallocated and
  ...
  it('does NOT pre-include the next unallocated ODS successor addendum (no automatic inclusion; the literal reserves nothing)', () => {
    expect(IMMUTABLE_BY_CONVENTION).not.toContain('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.37.json')
  })

  // APPEND-OVERSHOOT CONTROL (the successor enumerating v1.0.36): the absence
  // assertion above is a membership test. This one pins the BOUNDARY, so an
  // append beyond the forty-seventh entry cannot pass by simply not being
  // v1.0.37. Length is exact and the final slot is pinned by index, so
  // appending ANY further entry fails here even if the absence literal above
  // were never updated.
  it('APPEND OVERSHOOT: the list ends at exactly 47 entries with v1.0.36 in the final slot', () => {
    expect(IMMUTABLE_BY_CONVENTION.length).toBe(47)
    expect(IMMUTABLE_BY_CONVENTION[46]).toBe('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.36.json')
    expect(IMMUTABLE_BY_CONVENTION[47]).toBeUndefined()
    expect(IMMUTABLE_BY_CONVENTION.filter((e) => e.includes('v1.0.37'))).toEqual([])
  })
  ...
  // ODS_V1_MAINTENANCE_ADDENDUM_v1.0.36.json CONTROLLER_SUCCESSOR_RULE
  // .requirement ("The NEXT Controller successor act is REQUIRED to enumerate
  // docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.36.json in
  // IMMUTABLE_BY_CONVENTION, taking the THEN-LIVE array length to length PLUS
  // EXACTLY ONE, making v1.0.36 FINAL, and preserving the existing order of
  // every predecessor entry"): the Controller must enumerate its own governing
  // addendum, and that enumeration is a SEPARATE governed maintenance mission
  // from the addendum that allocated the lineage. v1.0.36 is explicitly NOT
  // self-including, and its CONTROLLER_SUCCESSOR_RULE.NOT_ALLOCATED_HERE clause
  // forbade that lane from touching scripts/ods-controller.ts or this file at
  // all. These controls prove the newly appended entry is actually WIRED INTO
  // decideSelection through the REAL guard, not merely present as a string —
  ...
  //
  // THIS BLOCK INVERTS TWO CONTROLS that stood here before this act. While
  // v1.0.36 was the absence-control literal, the suite asserted it was NOT
  // protected and that a node writing it remained SELECTABLE. Enumerating it
  // necessarily makes it protected and unselectable, so those two controls
  // ADVANCE to v1.0.37 (below) and these four take their place for v1.0.36.
  // The inversion is the substance of the enumeration, not a regression in it:
  // v1.0.36 CONTROLLER_SUCCESSOR_RULE
  ...
  // v1.0.36 onto the CONTROLLER axis for the first time.
  it('a node targeting v1.0.36 STOPs with PROTECTED_SURFACE_CHANGE via real decideSelection', () => {
    const unit = baseUnit({ writePaths: ['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.36.json'] })
    const decision = decideSelection(unit, 'OPEN', {}, {})
    expect(decision.selectable).toBe(false)
    expect(decision.stopClass).toBe('PROTECTED_SURFACE_CHANGE')
  })

  it('a case-mutated spelling of the new entry (v1.0.36) is NONCANONICAL_PROTECTED_PATH, never PROTECTED_SURFACE_CHANGE', () => {
    const unit = baseUnit({ writePaths: ['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_V1.0.36.JSON'] })
    const decision = decideSelection(unit, 'OPEN', {}, {})
    expect(decision.selectable).toBe(false)
    expect(decision.stopClass).toBe('NONCANONICAL_PROTECTED_PATH')
  ...
  // decideSelection. A guard that silently lowercased would report
  // violated=true here and fail.
  it('checkImmutableGuard classifies the case-mutated v1.0.36 spelling as noncanonical, never as a violation', () => {
    const guard = checkImmutableGuard(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_V1.0.36.JSON'])
    expect(guard.violated).toBe(false)
    expect(guard.violatingPaths).toEqual([])
    expect(guard.nonCanonicalPaths).toEqual(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_V1.0.36.JSON'])
  })
  ...
  // above this proves the guard distinguishes canonical from case-mutated
  // rather than answering the same way to both.
  it('checkImmutableGuard treats the canonical v1.0.36 spelling as a real protected-surface violation', () => {
    const guard = checkImmutableGuard(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.36.json'])
    expect(guard.violated).toBe(true)
    expect(guard.violatingPaths).toEqual(['docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.36.json'])
    expect(guard.nonCanonicalPaths).toEqual([])
  })

  // NON-ALLOCATION BEHAVIOURAL CONTROL (the successor enumerating v1.0.36,
  // ADVANCED from v1.0.36): the absence-control literal v1.0.37 is NOT
  ...
  it('v1.0.37 is NOT protected — being the absence-control literal confers no protected status', () => {
  ...
  // end-to-end demonstration that v1.0.37 is neither allocated, reserved nor
  ...
  it('a node whose writePaths is exactly v1.0.37 remains SELECTABLE through real decideSelection (not reserved, not protected)', () => {
  ...
  // this control from v1.0.35 to v1.0.36 does not weaken it: v1.0.35 remains
  // pinned by exact position inside the OLD46 literal and its ordered digest
  // below, so a duplicated v1.0.35 still fails the reconstruction controls.
  it('the new entry appears exactly once, and the list carries no duplicates at all', () => {
    const occurrences = IMMUTABLE_BY_CONVENTION.filter(
      (entry) => entry === 'docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.36.json',
    ).length
  ...
  // ---------------------------------------------------------------------
  // ORDER PROOF (ODS_V1_MAINTENANCE_ADDENDUM_v1.0.36.json
  // CONTROLLER_SUCCESSOR_RULE): an independently typed literal of the
  // pre-append 46-entry closed world (OLD46 — never derived from the live
  // IMMUTABLE_BY_CONVENTION import, and never copied from the candidate diff
  // that appends v1.0.36) is hashed with an ordered digest. A remove-only
  // reconstruction of the live, post-append list (dropping exactly the new
  // final element) must reproduce OLD46 element-by-element AND by that same
  ...
  // ---------------------------------------------------------------------
  describe('ORDER PROOF: append-only reconstruction of OLD46', () => {
    // Independently typed — extracted mechanically from the pre-v1.0.36
    // source (the BASE state of scripts/ods-controller.ts at
    // e7df0c9a558de0481f966008f0908bb3ee084811) via
    ...
    //
    // CORROBORATION (independent of this lane): v1.0.36.json states the same
    ...
    // length 46, no duplicates, v1.0.35 last.
    const OLD46: readonly string[] = [
    ...
    // either OLD46 above or the live pre-append 46 entries drifted — never
    ...
    // reproducing BOTH inherited pins below from OLD46.slice(0, -1) and
    // OLD46.slice(0, -2) before this value was pinned. Neither inherited
    // value was copied from its author's work on trust; each was re-derived
    // here from OLD46 and only then compared against the inherited pin.
    const OLD46_DIGEST_EXPECTED = 'c839a7e0123304c4d16af3168a2ac90dbf2f3d1b9d13ef3adba5f54d151e7e47'

    function orderedDigest(entries: readonly string[]): string {
      return crypto.createHash('sha256').update(JSON.stringify(entries)).digest('hex')
    }

    it('OLD46 literal has length 46, no duplicates, and matches the pinned OLD46_DIGEST', () => {
      expect(OLD46.length).toBe(46)
      expect(new Set(OLD46).size).toBe(46)
      expect(orderedDigest(OLD46)).toBe(OLD46_DIGEST_EXPECTED)
    })

    it('the live list is exactly OLD46 with v1.0.36 appended as the sole new final element', () => {
      expect(IMMUTABLE_BY_CONVENTION.length).toBe(47)
      expect(IMMUTABLE_BY_CONVENTION[46]).toBe('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.36.json')
    })

    it('REMOVE-ONLY RECONSTRUCTION: dropping the final element of the live list reproduces OLD46 element-by-element, in order', () => {
      const reconstructed = IMMUTABLE_BY_CONVENTION.slice(0, -1)
      expect(reconstructed).toEqual(OLD46)
      expect(reconstructed.length).toBe(OLD46.length)
      for (let i = 0; i < OLD46.length; i++) {
        expect(reconstructed[i]).toBe(OLD46[i])
      }
    })

    it('REMOVE-ONLY RECONSTRUCTION: its ordered digest matches OLD46_DIGEST exactly (catches any predecessor reorder)', () => {
      const reconstructed = IMMUTABLE_BY_CONVENTION.slice(0, -1)
      expect(orderedDigest(reconstructed)).toBe(OLD46_DIGEST_EXPECTED)
    })

    // CONTINUITY CONTROL (the successor enumerating v1.0.36): the immediately
    // preceding pin must still be reproducible from OLD46 by remove-only
    // reconstruction. This ties the new OLD46 literal back to the
    // independently-audited predecessor pin, so OLD46 cannot have silently
    // drifted in any of its 45 inherited entries while still satisfying its
    // own freshly-pinned digest. The pin below is the value the predecessor
    // act pinned as its own 45-entry digest constant; it is reproduced here,
    // not inherited on trust.
    //
    // THIS CONSTANT CARRIES NO ORDINAL, AND THE ONE BELOW IT KEEPS ITS OWN.
    // The act that pinned the 45-entry digest took no ordinal — v1.0.35
    ...
    it('CONTINUITY: OLD46 minus its final element reproduces the inherited OLD45 pin exactly', () => {
      const priorClosedWorld = OLD46.slice(0, -1)
      expect(priorClosedWorld.length).toBe(45)
      expect(OLD46[45]).toBe('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.35.json')
      expect(orderedDigest(priorClosedWorld)).toBe(OLD45_DIGEST_INHERITED_PIN)
    })

    // DEEPER CONTINUITY (two generations back): the same remove-only walk
    // reproduces the 44-entry pin. Chaining two inherited pins rather than one
    // means a drifted inherited entry would have to satisfy BOTH
    // independently-audited historical digests to pass unnoticed.
    const OLD44_DIGEST_INHERITED_PIN = '1248eb2ca1b03d49f0da68739e961770307187bd17251e611877319dfd8a4ce8'

    it('CONTINUITY: OLD46 minus its final two elements reproduces the inherited OLD44 pin exactly', () => {
      const twoBack = OLD46.slice(0, -2)
      expect(twoBack.length).toBe(44)
      expect(OLD46[44]).toBe('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.34.json')
      expect(orderedDigest(twoBack)).toBe(OLD44_DIGEST_INHERITED_PIN)
    })
    ...
    // construction. Swapping two adjacent OLD46 entries must change the
    ...
    it('MUTATION CONTROL: reordering two predecessor entries changes the ordered digest (order-blind checks would miss this)', () => {
      const reordered = [...OLD46]
      const tmp = reordered[0]
      reordered[0] = reordered[1]
      reordered[1] = tmp
      expect(reordered.length).toBe(OLD46.length)
      expect(new Set(reordered)).toEqual(new Set(OLD46))
      expect(orderedDigest(reordered)).not.toBe(OLD46_DIGEST_EXPECTED)
    })

    // ORDER-CONTINUITY CONTROL (M2 class, in-suite): the controls above are
    // stated over OLD46. This one is stated over a SIMULATED LIVE list — the
    ...
    // between a reorder and a green suite. Every index below is <= 45, i.e.
    // strictly inside the INHERITED range, so the appended final element at
    // index 46 is never the thing being moved.
    ...
    it.each([
      ...
      ['maximally distant inherited entries', 0, 45],
      ['two mid-list inherited addenda', 20, 44],
      ...
      expect(tampered[46]).toBe('docs/ops/ods/ODS_V1_MAINTENANCE_ADDENDUM_v1.0.36.json')

      // The order-sensitive reconstruction is not.
      const reconstructed = tampered.slice(0, -1)
      expect(reconstructed).not.toEqual(OLD46)
      expect(orderedDigest(reconstructed)).not.toBe(OLD46_DIGEST_EXPECTED)
    })
    ...
    it('ORDER CONTINUITY non-vacuity: the same pipeline with no swap reconstructs OLD46 and its digest', () => {
