...
export function extractDocument(buffer: Buffer, mimeType: string): ExtractionResult {
  // Normalize away MIME parameters ("text/csv; charset=utf-8" -> "text/csv").
  ...
  const gatedLabel = G5_GATED_FORMATS[mime]
  if (gatedLabel) {
    return emptyResult('unsupported', [
      `${gatedLabel} extraction (${mime}) requires an npm dependency pending the G5 product decision (docs/ops/gates/G5_PACKAGE.md); binary formats are never hand-parsed`,
    ])
  }

  return emptyResult('unsupported', [`No extractor registered for MIME type "${mime}"`])
}
