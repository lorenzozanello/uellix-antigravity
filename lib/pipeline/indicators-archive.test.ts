// lib/pipeline/indicators-archive.test.ts
// FIBIU-03 — indicators gain a lifecycle state and an archive path.
...
import { archiveIndicator } from './indicators'
...
const ACTIVE_INDICATOR = { id: 'ind-1', projectId: 'proj-1', status: 'active' }

describe('archiveIndicator', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.mocked(getCurrentOrganizationContext).mockResolvedValue(CTX as never)

    // where() is called for: project lookup, existing-indicator lookup,
    // update's own where, then the post-update re-select. Wire it up as a
    // small sequential queue keyed by call order.
    let call = 0
    whereMock.mockImplementation(() => {
      call += 1
      return { then: thenMock, limit: vi.fn().mockReturnThis() }
    })
    let thenCall = 0
    thenMock.mockImplementation((cb: (rows: unknown[]) => unknown) => {
      thenCall += 1
      if (thenCall === 1) return Promise.resolve(cb([PROJECT]))
      if (thenCall === 2) return Promise.resolve(cb([ACTIVE_INDICATOR]))
      return Promise.resolve(cb([{ ...ACTIVE_INDICATOR, status: 'archived' }]))
    })
  ...
  it('sets status to archived and records who/when', async () => {
    const result = await archiveIndicator('proj-1', 'ind-1')
  ...
  it('writes a governed audit event for the archive transition', async () => {
    await archiveIndicator('proj-1', 'ind-1')
    expect(logAuditAction).toHaveBeenCalledWith(
      expect.objectContaining({ entityType: 'indicator', action: 'indicator.archived' })
    )
  })
  ...
  it('rejects archiving below analyst', async () => {
    ...
    await expect(archiveIndicator('proj-1', 'ind-1')).rejects.toThrow('Insufficient permissions')
  })
})
